...
NO_LIMIT = {"id": "unconstrained", "name": "Unconstrained (No Limits)",
            "ruleset_type": "personal", "max_loss_eod": 0}
...
def test_a_failed_phase_never_also_claims_it_was_not_run():
    ...
    _, reasons = compute_grade(_st(sensitivity_max_degradation=0.05), None, {"p": {}},
                               LIMITED, wf_failed=True)
...
def test_a_failed_sensitivity_is_reported_and_neutral():
    ...
    assert any("sensitivity was requested and FAILED" in r.lower() or "FAILED" in r for r in reasons)
...
def test_prob_pass_is_measured_on_the_same_basis_as_prob_breach():
    ...
    ruleset = {"id": "eval", "ruleset_type": "prop_eval", "account_size": 10_000,
               "max_loss_eod": 6000, "profit_target": 100}
    mc = run_monte_carlo(pnls, ruleset, num_reshuffles=500, num_bootstrap=500,
                         num_path_samples=5, balances=balances)
...
def test_the_percent_histogram_exists_exactly_when_the_percent_basis_does():
    ...
    pnls, balances = _compounding_run()
    mc = run_monte_carlo(pnls, None, num_reshuffles=200, num_bootstrap=200,
                         num_path_samples=5, balances=balances)
    assert mc["dd_basis"] == "percent"
...
def test_a_param_behind_a_switch_this_run_has_off_is_not_perturbed():
    ...
    assert [p["name"] for p in perturbable_params({"param_schema": schema}, off)] == ["aplus_window"]
...
def test_the_simulation_counts_are_bounded():
    ...
    for bad in ({"num_simulations": 10_000_000}, {"num_bootstrap": 0},
                {"walk_forward_windows": 500}, {"walk_forward_windows": 1}):
...
def test_the_requested_phases_are_stored_when_the_row_is_inserted(fresh_db):
    ...
    lab_db.insert_stress_test({
        "stress_test_id": "st_new", "run_id": "src", "ruleset_id": None,
...
def test_a_row_created_without_the_field_reads_as_unknown_not_as_monte_carlo_only(fresh_db):
    ...
    _seed_running_test()
    lab_db.insert_stress_test({"stress_test_id": "st_old", "run_id": "src", "ruleset_id": None,
                               "status": "running", "created_at": 0})
    ...
    with lab_db._connect() as conn:
        ...
        for rid, status in (("child_running", "running"), ("child_done", "complete")):
            conn.execute(
                ...
                "'2021-01-01',0,0,?,1,'st1')", (rid, status))
    return "st1"
