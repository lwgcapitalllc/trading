...
🔴 **AND `turn + _CONFIRM_BARS` WAS NOT THE END OF THE ZONE EITHER (2026-08-08).** The bar that makes
the adverse extreme can sit anywhere in the drawdown, so on a trade that tops out early and then
grinds sideways above its entry, the span covered a fraction of what the reader sees as the red
band. ✅ **MEASURED on the 2026-06-18 short: turn at 02:00, still above entry until 05:30 — 21 bars
of drawdown against a 9-bar span, holding THREE patterns of which ONE was drawn**, and the one drawn
was the opposing candle, so the chip read `no matching candle`. `_drawdown_end` walks from the turn
while the bar still trades on the adverse side of the ENTRY PRICE and stops at the first bar lying
entirely on the favourable side. ✅ **MEASURED over 166 anchors: 846 → 1090 marks.**

🔴 **AND THE ZONE IS A PRICE BAND, NOT A STRETCH OF TIME — the RANGE alone left 23% of the marks
outside it (2026-08-09).** Aaron, off a winning long reading `Won · Inverted Hammer`: *"there is no
inverted hammer near entry or within draw down… it is somewhere in between… Once price takes off
into our favor, we should not be trying to highlight any candles."* ✅ **MEASURED on run
`e51d95f212e3`: 255 of 1084 marks sat ENTIRELY on the favourable side of the entry**, in two places
with two different causes:

  - **35 in the confirm tail.** `_CONFIRM_BARS` was being added to `_drawdown_end`'s answer rather
    than to the TURN. The allowance exists because a pattern completes on its LAST bar, so it is
    measured from the extreme and means nothing anywhere else — added to the end of the drawdown it
    is two free bars after price has already left the band. On his 2026-07-15 long the drawdown ends
    17:15, the next bar rallies $16 clear of the entry, and the `Inverted Hammer` two bars later at
    4060-4066 — **$25 above the entry** — was the trade's ONLY mark, so it named the chip.
  - **196 BEFORE the turn, which no bound on the END could ever have caught.** The span opens at the
    entry BAR, so a trade that runs into profit first and only later comes back to make its adverse
    extreme painted the whole excursion in between — **on one trade the turn is 112 bars after
    entry.**

`_reversal_span` anchors the confirm allowance on the turn and `_in_zone` then filters bar by bar:
a bar qualifies while ANY part of it trades on the adverse side of the entry, which is the same test
as *does this candle sit in the red band*. ✅ **MEASURED, driven through the live backend: 1090 → 861
marks, 36 of them outside the band and every one within `_CONFIRM_BARS` of the extreme; anchors
carrying a reversal candle 149 → 145 of 166.** Aaron's own trade now carries **none**, which is the
honest answer — its drawdown holds no pattern at all, and the `Inverted Hammer` was never in it.

⚠ **BOTH bounds are load-bearing and neither is sufficient.** The RANGE stops a winner that retraces
to its entry hours later from re-opening the zone (the exit bar of that same long dips back through
the entry); the per-bar BAND test stops a favourable stretch INSIDE the range from being painted.
⚠ **The confirm bars are exempt from the band test, and that exemption is why this is not simply a
price filter** — on a sharp V the third bar of a Morning Star is already back above the entry, and
that bar IS the reversal candle. ✅ **MEASURED: a band-only rule leaves 40 trades with no mark
against this rule's 21.**
⚠ **Nothing is drawn once price has left the band**: after that the trade is winning, and a reversal
candle there is a different subject — it would put navy candles all down a runner's profitable leg.
⚠ **A 3/3 MISS passes NO entry price, so every bar of its span qualifies**: no position was opened,
so there is no entry to measure a band from, and its span is already the visit into the zone.
...
computed** — the second can only ever agree with itself. The span now covers the whole drawdown
(`_drawdown_end`), and on that trade the layer draws all three.
...
`tests/test_candle_overlays.py` (44) + `tests/test_chart_spec_reversal_anchors.py` (9). ⚠ **A
