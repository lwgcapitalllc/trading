...
| Lab — Firms | ✅ Live | Prop firm rule profiles (drawdown limits, profit targets, consistency %). CRUD endpoints. Firms: 4 LucidFlex (50k/100k × eval/funded) + 4 Tradeify Select (50k/100k × eval/funded). Schema extended with `eval_cost_usd`, `activation_fee_usd`, `profit_split_pct` columns. `max_contracts` carries optional free-form `scaling` object (no schema change needed — stored as JSON dict). Tradeify funded rows use `cumulative_ratchet` scaling; LucidFlex funded rows use `bidirectional_band` scaling. LucidFlex rows corrected 2026-05-31: `drawdown_type` -> `trailing_eod` (was `eod`/flat), `force_flat_time_et` -> `16:45` (was `15:30`), eval contracts set to confirmed fixed full size (50k 4/40, 100k 6/60), funded bidirectional scaling added. TODO: verify whether LucidFlex 50k/100k carry a daily loss limit (DLL) in eval + early funded — `max_loss_intraday` left NULL pending confirmation. |
