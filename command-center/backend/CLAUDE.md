...
**MT5 deal direction vs position direction:** MT5 Strategy Tester emits 2 deal-rows per trade — an entry deal (profit=0, deal direction = position direction: "buy" for Long, "sell" for Short) and an exit deal (profit=realized P&L, deal direction = opposite of position direction: "buy" to close a Short, "sell" to close a Long). `_normalize_mt5_results` must NOT map deal direction to position direction naively. It pairs entry+exit deals in time order (entry = profit==0.0, exit = profit≠0), takes the position direction from the entry deal's direction field, and only marks the EXIT deal on the equity curve. This produces one equity curve direction point per trade with the correct position direction — used by the Long vs Short breakdown in BacktestDetail. Do not revert to mapping all deals: that produces double the trade count and inverted direction labels on exit deals.
