...
⚠ **In memory, per backend process.** Two clones each run their own backend and do not see each
other's claims — the box itself is the only thing both share, and it has no lock to offer. A
backend restart clears every claim, which is right: whatever was running died with it (a deploy's
own record says so — see `promote_jobs.py`).
...
def claim(bot_key: str, doing: str, name: str | None = None) -> None:
    ...
    with _lock:
        ...
        if held is not None:
            raise Busy(_sentence(name or bot_key, held[0]))
...
def account_busy(bot_keys: Iterable[str], names: dict[str, str] | None = None) -> str | None:
    ...
    names = names or {}
    with _lock:
        busy = [(k, _ops[k][0]) for k in bot_keys if k in _ops]
    if not busy:
        return None
    k, what = busy[0]
    return (
...
def snapshot() -> dict[str, str]:
    ...
    with _lock:
        return {k: v[0] for k, v in _ops.items()}
