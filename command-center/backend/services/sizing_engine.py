...
from services.decision_log import SizingDecision, TradeDecision, classify_exit
...
class RawTrade:
    ...
    breakeven_time: Optional[datetime] = (
        None  # when the stop reached BE (future; unused — see note)
    )
    ...
    def _scaling_cap(self, scaling: dict, profit: float) -> Optional[float]:
        ...
        if mode == "cumulative_ratchet":
            ...
            if ceil and ceil.get(self._key) is not None:
                cap = (
                    min(cap, float(ceil[self._key])) if cap is not None else float(ceil[self._key])
                )
            return cap
...
class EngineRuleset:
    ...
    def from_ruleset(cls, r: dict, is_micro: bool) -> "EngineRuleset":
        return cls(
            ...
            mll_lock_balance=(
                float(r["mll_lock_balance"]) if r.get("mll_lock_balance") is not None else None
            ),
            consistency_pct=(
                float(r["consistency_pct"]) if r.get("consistency_pct") is not None else None
            ),
            daily_loss_limit=(float(r["daily_loss_cap"]) if r.get("daily_loss_cap") else None),
            risk_per_trade_pct=(
                float(r["risk_per_trade_pct"]) if r.get("risk_per_trade_pct") else None
            ),
            daily_halt_fraction=(
                float(r["daily_halt_fraction"]) if r.get("daily_halt_fraction") else None
            ),
            daily_profit_target=(
                float(r["daily_profit_target"]) if r.get("daily_profit_target") else None
            ),
            max_dd_from_peak_pct=(
                float(r["max_drawdown_from_peak_pct"])
                if r.get("max_drawdown_from_peak_pct")
                else None
            ),
...
def run_engine(
    ...
    *,
    ...
    result.risk_budget_note = f"mode={mode}; " + (
        f"manual = {manual_risk_pct}% of balance per trade"
        if mode == MODE_MANUAL
        else "bullet = max size the rules allow"
        if mode == MODE_BULLET
    ...
    for day in days:
        ...
        for _ts, kind, t in events:
            if kind == 0:  # ── ENTRY ──
                dec = TradeDecision(
                    timestamp=_iso(t.entry_time),
                    instrument=instrument,
                    direction=t.direction,
                    strategy=strategy,
                    account_id=account_id,
                    signal_id=str(t.index),
                    stop_distance=t.stop_distance,
                    proposed_stop=t.stop_price(),
                )
                ...
                if available is not None and available <= 0:
                    ...
                    dec.set_sizing(
                        SizingDecision(
                            contracts=0,
                            bound_by="insufficient_room",
                            room_to_floor=room,
                            skipped=True,
                            note="no room left after open-trade reservation",
                        )
                    )
                    result.blocked_trades += 1
                    continue
                ...
                if contracts <= 0:
                    ...
                    result.sized_trades.append(
                        SizedTrade(
                            ...
                            net_pnl=0.0,
                            bound_by=bound,
                            risk_per_contract=t.risk_per_contract(),
                            skipped=True,
                        )
                    )
                    continue
                ...
                dec.set_exit(
                    time=_iso(t.exit_time),
                    price=t.exit_price,
                    reason=classify_exit(t.exit_reason),
                    gross=round(t.gross_per_contract() * contracts, 2),
                    net=round(net, 2),
        ...
        if floor_eod is not None and not breached and eod_balance <= floor_eod:
            ...
            result.breach_reason = (
                "trailing_max_loss" if rs.has_trailing_floor else "drawdown_from_peak"
            )

        share = (
            (day_pnl / cumulative_profit * 100.0)
            if (day_pnl > 0 and cumulative_profit > 0)
            else None
        )
        ...
        if rs.has_trailing_floor:
            ...
            trailing_floor = (
                max(trailing_floor, candidate) if trailing_floor is not None else candidate
            )
        ...
        result.timeline.append(
            DayTimeline(
                ...
                contracts_total=day_contracts,
                day_pnl=round(day_pnl, 2),
                eod_balance=round(eod_balance, 2),
                risk_floor=(round(floor_eod, 2) if floor_eod is not None else None),
                floor_distance=(
                    round(eod_balance - floor_eod, 2) if floor_eod is not None else None
                ),
