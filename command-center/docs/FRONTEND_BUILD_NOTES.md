...
translated out of the backend's own vocabulary (`solo:b_leg` → *Replaying B-LEG on its own
account*).
...
| print `p.message` raw instead of the words | the banner reads `solo:b_leg` |
