...
| Backtests lab M2 — parameter optimizer (native NT8/MT5 optimizer, Optimization Detail with ranked results table, ★ best row) | ✅ |
| Backtests Speed Step 1 — native NT8 optimizer path (`search_method="native"`): one VPS job, all CPU cores, export CSV results grid, create run rows on return. VPS: `nt8_backtest_runner.run_native_optimize_mode` switches SA to Optimization mode, sets param ranges as `lo;hi;step` format (fixed params use `value;value;1`), exports results. Agent: `POST /native-optimize`, `GET /jobs/{id}/native-opt-results`, `GET /optimize-mode-dump?strategy=X` diagnostic. Backend: `run_native_optimization()` in `optimization_runner.py`. Native is the only search method — brute/genetic removed. No auto-trigger stress test (no per-combo equity curve). Parity confirmed: ORB ORMinutes=50 TpMultiple=5 → NT8 optimizer and single-run paths agree to $0.01. | ✅ |
| Backtests Speed Step 2 — re-score native opt results with real objective: All combos returned by NT8 are stored (wide cut removed — user decides manually which combos to run full backtests on). `estimated_runs` updated to actual NT8 combo count after grid completes (NT8 silently drops zero-trade combos from its CSV, so the count returned can be less than the grid size). (1) Drawdown substitution: NT8 "Max. drawdown" is cumulative peak-to-trough — not comparable to prop firm's daily `max_loss_eod`. `run_native_optimization` substitutes `MaxDailyLoss` from `fixed_params` as the effective per-period drawdown when evaluating and tiering native combo rows. (2) Win-rate fix: NT8 CSV already reports as fraction (0.327); VPS parser no longer divides by 100 when value < 1. Scoring via `eval_pass_probability` / `funded_sharpe` / `raw_pf` per mode — winner picked by our objective, not NT8's built-in rank. Worthiness Tier 1/2/3 auto-assigned per combo. | ✅ |
...
| Backtests lab M4 — active job indicator: tab-specific pulsing dots. Runs dot = any `status=running` run that is not a sweep child or optimization combo (includes opt-combo full backtests while they are active). Sweeps dot = any sweep with `status=running`. Optimizations dot = any optimization grid with `status=running` (NOT triggered by a single-combo full backtest — `decrement_optimization_completed` no longer sets `status=running` for single-combo retries). | ✅ |
...
| Backtests Speed Step 3 — native opt powers stress testing | ✅ | **(A) Grid sensitivity:** `_compute_grid_sensitivity()` in `optimization_runner.py` runs on all combos; measures PF degradation for winner's neighbors in each ranged param; stores `grid_sensitivity_score` + `grid_sensitivity_summary` on the optimization row. `stress_tester._apply_grid_sensitivity_if_available()` populates sensitivity fields from the opt row (no NT8 backtests) when source run has `optimization_id`. **(B) Manual full backtest on winner:** Auto-trigger of winner backtest removed. User views results in OptimizationDetail, picks a combo, clicks "Full Backtest" on BacktestDetail to get equity curve + trade data. `retry_single_optimization_run` handles this path; keeps the parent optimization `status=complete` (no confusing Opts-tab pulse); writes progress to `lab_progress.json` via `_poll_one(write_progress=True)` so progress bar works. Running opt-combo backtests appear in the Runs tab while active. **(C) Native walk-forward:** `run_native_walkforward_mode()` in `nt8_backtest_runner.py` sets BacktestType="Walk Forward", all params as `value;value;1` (no IS re-optimization), configures WF periods/OOS%; exports and parses WF results CSV. Agent: `POST /native-walkforward` + `GET /jobs/{id}/native-wf-results`. `stress_tester.run_walk_forward_task()` dispatches to `_run_native_walk_forward()` when source run has `optimization_id`. **WF auto_ids need VPS validation** (`OutOfSampleIterations`, `OutOfSamplePercentage`). |
| Backtests Speed Step 4 — MT5 native optimizer + walk-forward | ✅ | MT5 agent `_run_mt5_optimization()` uses **sequential single backtests** (one per combo, `Optimization=0`). Root cause: MT5 `Optimization=1` command-line mode does not write a parseable results file — it only populates the GUI results tab. Fix: expand the param grid in Python, run each combo as a regular `Report=`-based backtest (the mechanism already validated for single runs), parse each `o{job_id}_{i}.htm`. `_run_mt5_forward_test()` uses `ForwardMode` in ini (2=50%OOS, 3=33%, 4=25%), parses IS and OOS KPI sections split at "forward" keyword boundary. Agent endpoints: `POST /native-optimize`, `GET /backtests/{id}/native-opt-results`, `POST /native-walkforward`, `GET /backtests/{id}/native-wf-results`. `mt5_agent_client.py` exposes matching typed wrappers. `nt8_agent_client` dispatcher updated: `start_native_optimization`, `native_opt_results`, `start_native_walkforward`, `native_wf_results` all accept `runner` param and route to MT5 client when `runner="mt5"`. `optimization_runner.run_native_optimization` reads `runner_str` from strategy and passes it through the entire poll loop. |
...
| MT5 backtest detail | ✅ Live | `BacktestDetail` is MT5-aware via `runner` field on API response (`BacktestDetail` model + `_row_to_detail`): `MT5_RUN_STEPS` (Launch→Testing→Results→Tagging) vs `NT8_RUN_STEPS` (Connect→Configure→Run→Results→Tagging); runner-aware failure messages; "Load chart data from NT8" and "Refresh" buttons hidden for MT5; Stress Test button visible for all completed runs with trades; empty-state copy updated; `_normalize_mt5_status` returns `pct=30` + "MT5 Strategy Tester running…" while running. **KPI completeness (2026-06-06):** `_normalize_mt5_results` computes `avg_win`/`avg_loss` from trades list. Direction/profit injected into equity curve points so `DirectionBreakdown` works. **Date parsing (2026-06-06):** MT5 equity dates are full ISO datetimes (`2025-12-11T18:51:00`) — `calTickLabel`, `calIndexTicks`, `fmtChartDate`, and `computeRegimeBands` all slice to 10 chars before parsing to avoid NaN axis labels, invalid tooltip dates, and missed regime lookups. Calmar also slices. Daily P&L hover cursor `rgba(255,255,255,0.04)`. |
| Run Stress Test modal | ✅ Live | `RunStressTestModal` in `BacktestDetail.tsx`. No checkboxes — WF + sensitivity always run together. Ruleset locked to `run.evaluations[0]?.ruleset_id` (readonly amber chip or "Monte Carlo only"). Fixed estimate: native WF path (`run.optimization_id && runner !== 'mt5'`) → ~45 min; all others → ~80 min. Always sends `include_walk_forward: true, include_sensitivity: true`. On success navigates to `/stress-tests/{stress_test_id}`. Button shows pulsing "In progress" (navigates to stress test detail) when a stress test is already running; reverts to modal-opener when idle. |
| Stress test market lock | ✅ Live | Only one stress test per market type (futures/forex) may run at a time. Backend: `lab_db.running_stress_test_markets()` joins `stress_tests` → `backtest_runs` → `strategies`; returns `{futures, forex, run_ids}`; `GET /stress-tests/running-lock` exposes it; `POST /stress-tests/run` checks the market lock before inserting (409 if blocked); lock SQL uses `LIKE 'running%'` to cover `running`, `running_wf`, and `running_sens`. Frontend: `useRunningStressLock()` in `useStressTests.ts` polls at 5s; `stressBlocked = isMt5 ? lock.forex : lock.futures`; Stress Test button disabled with tooltip when blocked. |
| Running stress test indicators | ✅ Live | **Runs tab:** `RunsTab` builds `stressRunIds` from `lock.run_ids`, passes `hasRunningStress` to each `RunRow` (pulsing cyan "STRESS TESTING" chip) and `hasRunningStress` to each `OptimizationNestRow` (same chip; checked against `opt.best_run_id`). `OptimizationNestRow` no longer shows the instrument (redundant with the parent run row). **BacktestDetail:** Stress Test button transforms to a pulsing "In progress" button (navigates to stress test) when `latestStress` is in a `running*` state; no separate banner. **OptimizationDetail:** `useRunningStressLock` + `useStressTests(bestRunId)` — clickable "Stress test in progress on winner run" banner when `stressRunIds.has(opt.best_run_id)`; navigates to the stress test. |
| Stress test child run isolation | ✅ Live | `lab_db.list_runs()` always appends `r.stress_test_id IS NULL` to its WHERE clause — walk-forward window runs and sensitivity perturbation runs never appear in the Runs tab. They remain in the DB and are accessible from `StressTestDetail`. |
| Stale stress test recovery | ✅ Live | `lab_db.reset_stale_stress_tests()` — marks any stress test with `status LIKE 'running%'` as `failed_crashed` and its running child backtest_runs as `failed_timeout`. Called in `main.py` `startup()` so backend restarts automatically clear stuck tests. Prevents the market lock from staying asserted indefinitely after a crash. |
| Strategy best grades | ✅ Live | `GET /stress-tests/strategy-grades` → `lab_db.best_grades_by_strategy()` returns `{strategy_id: {grade, stress_test_id}}` for the best completed grade per strategy (A > B > C > D > F). Frontend: `useStrategyBestGrades()` hook (polls 30s). Strategies tab has a "Best Grade" column showing a `RobustnessGradeBadge`; clicking it navigates to the stress test that produced the grade. Rows with no graded stress test show `—`. |
...
| OptimizationDetail 3-view toggle (2026-06-08) | ✅ Live | Results section has a segmented **Table \| Bar Chart \| Heatmap** toggle. Table: sortable ranked table, always available. Bar Chart (`RankedBars` inline component in `OptimizationDetail.tsx`): horizontal bars sorted by PF descending, each bar labeled with swept param values (e.g. `20 / 7`), colored by worthiness tier (green/cyan/red), winner gets gold border + ★. Heatmap: 2D color grid — shown for 2 swept params; shows an informational message for other counts. All three views clickable (navigate to run detail). CSV export always visible. |
| Full Backtest on opt combo — bug fixes (2026-06-08) | ✅ Live | Four bugs fixed when user clicks "Full Backtest" from an optimization combo's BacktestDetail: **(1) Progress bar** — `_poll_one` in `optimization_runner.py` now accepts `write_progress=True` (set by `retry_single_optimization_run`) and forwards VPS agent `pct`/`message` to `lab_progress.json` via `write_job_progress()` in `backtest_runner.py`. **(2) Runs tab visibility** — running opt-combo runs are no longer filtered out of the Runs tab (filter changed from `!r.optimization_id` to `!r.optimization_id \|\| r.status === 'running'`); they appear with their OPT chip linking back to the parent optimization, disappear again once complete. **(3) Tab pulse** — `decrement_optimization_completed` no longer sets `status='running'` for single-combo retries (`set_running=False` param added); Optimizations tab dot only fires during an actual grid run. **(4) Regime tagging** — `_handle_opt_complete` in `optimization_runner.py` now runs `_tag_daily_pnl_with_regime` (imported from `backtest_runner`) before `update_run_complete`, same as the regular backtest pipeline. Previously opt-combo full backtests arrived with all UNKNOWN regime tags. **(5) Nesting** — Full Backtest row in Runs tab now renders immediately after the optimization that spawned it (keyed by `optimization_id`, with `pl-10` indent), not as a sibling at the end of all children. `optById` useMemo removed (was only needed for the old `source_run_id` lookup). |
| NT8 opt config speedup (2026-06-08) | ✅ Live | `run_native_optimize_mode` in `nt8_backtest_runner.py` previously called `_build_opt_grid_map(sa)` — a full `sa.descendants()` UIA tree scan — once per numeric fixed param (~10 foundational params × 30-60s each = 5-10 min config phase). Foundational params are in a collapsed SA category and always fall through to the PDEX fallback anyway, making every scan wasted. Fix: build grid_map once before the loop; catch stale-element exceptions and rebuild only for that param. Config phase now ~30-60 s total. |
| Optimization log persistence (2026-06-09) | ✅ Live | `run_native_optimization()` in `optimization_runner.py` persists the VPS agent log to `reports/lab/{opt_id}/opt_log.txt` at every exit path (success, failure, timeout, cancel). `GET /optimizations/{id}/log` falls back to that file when the agent's in-memory job record is gone (e.g. after agent restart). `OptLogSection` in `OptimizationDetail.tsx`: collapsible VPS Log panel at the bottom of the page, matches `BacktestDetail` `LogsSection` exactly — pulsing dot when running, lazy fetch, final refetch on run→done transition. |
| Copy buttons on log terminals (2026-06-09) | ✅ Live | Copy-all icon added to every log terminal: `LogsSection` in `BacktestDetail.tsx`, `OptLogSection` in `OptimizationDetail.tsx`, `LogModal` in `Bots/index.tsx`. Uses `navigator.clipboard.writeText`; icon flips to a checkmark for 1.5 s. Button only renders when log is non-null. |
| Optimization re-run + timer fix (2026-06-09) | ✅ Live | **Re-run button** appears on failed/cancelled optimizations (both NT8 and MT5). `POST /optimizations/{id}/rerun` resets the existing optimization in-place: clears all child runs and their files (`reset_optimization_for_rerun` in `lab_db.py`), sets `status=running / completed_runs=0 / completed_at=NULL / best_run_id=NULL`, then re-triggers `run_optimization(optimization_id)`. No new optimization is created — the user stays on the same detail page and sees it flip back to running. **Timer fix:** `useElapsed` in `OptimizationDetail.tsx` now freezes when `running=false`, using `completed_at` if available or `Date.now()` as a fallback. Previously the timer continued ticking indefinitely on failed runs when `completed_at` was null. |
