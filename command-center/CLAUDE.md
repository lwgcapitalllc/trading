...
| MT5 backtest detail | ✅ Live | `BacktestDetail` is MT5-aware via `runner` field on API response (`BacktestDetail` model + `_row_to_detail`): `MT5_RUN_STEPS` (Launch→Testing→Parse) vs `NT8_RUN_STEPS`; runner-aware failure messages; "Load chart data from NT8", "Refresh", and "Stress Test" buttons hidden for MT5; empty-state copy updated; `_normalize_mt5_status` returns `pct=30` + "MT5 Strategy Tester running…" while running. **KPI completeness (2026-06-06):** `_normalize_mt5_results` computes `avg_win`/`avg_loss` from the trades list (MT5 agent doesn't report these). Direction/profit injected into equity curve points at trade-close timestamps (`buy`→`Long`, `sell`→`Short`) so `DirectionBreakdown` (Long vs Short pie charts) works. Calmar: `computeCalmar` slices equity dates to `YYYY-MM-DD` — MT5 dates are full ISO datetimes; appending `T00:00:00` produced NaN. Daily P&L hover cursor set to `rgba(255,255,255,0.04)` (default Recharts cursor was opaque white, obscuring bars). Tag Regimes `onError` handler added — silent failures now surface as a toast. |
