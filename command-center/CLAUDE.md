...
NT8 and NinjaTrader were merged into one dot. Red = agent down (clickable); yellow = agent up but NT8 not running or Strategy Analyzer not open (needs RDP intervention); **grey "off" = NinjaTrader switched off on the VPS on purpose** — not clickable, the reason on the tooltip.

**Switching NinjaTrader off on purpose (2026-09-11).** The box holds the intent: the `NT8Agent` scheduled task **disabled** (or absent) means off. Off: `schtasks /change /tn NT8Agent /disable`, then stop the agent by its commandline (never `/im python.exe`). Back on: `schtasks /change /tn NT8Agent /enable`, `schtasks /run /tn NT8Agent`, then open NinjaTrader over RDP. The app notices within ~2 minutes and then stops restarting the agent, draws the grey dot, refuses NT8 jobs with the reason, and every failed NT8 call says why. ⚠ **No setting in this app** — a laptop's opinion about the VPS would disagree with the other clone. Rules: `backend/CLAUDE.md` → *NinjaTrader switched off on purpose*.
