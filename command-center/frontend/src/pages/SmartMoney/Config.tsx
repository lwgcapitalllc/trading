...
export function Config({ config, gitStatus, onSave, isSaving, saveError }: ConfigProps) {
  ...
  const lookbackOk =
    form.lookback_min_days <= form.lookback_preferred_days &&
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-bg-surface border border-border-subtle rounded-lg p-4">
          ...
          <NumberField
            ...
            onChange={(v) => set('min_trades', v)}
            min={1}
          />
          <NumberField
            label="Minimum win rate"
            description="per 30-day window (%)"
            value={form.min_win_rate_pct}
            onChange={(v) => set('min_win_rate_pct', v)}
          />
          <NumberField
          ...
          />
          <NumberField
            label="Min active weeks / month"
            value={form.min_active_weeks_per_month}
            onChange={(v) => set('min_active_weeks_per_month', v)}
            min={1}
          />
          <NumberField
            label="Max single-trade PnL share %"
            value={form.max_single_trade_pnl_share_pct}
            onChange={(v) => set('max_single_trade_pnl_share_pct', v)}
          />
          <NumberField
            label="Max avg hold time"
            description="hours"
            value={form.max_avg_hold_hours}
            onChange={(v) => set('max_avg_hold_hours', v)}
          />
          <NumberField
            label="Min account age"
            description="days"
            value={form.min_account_age_days}
            onChange={(v) => set('min_account_age_days', v)}
            min={1}
          />
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-bg-surface border border-border-subtle rounded-lg p-4">
            <div className="text-micro font-semibold uppercase tracking-[0.7px] text-text-secondary mb-3 flex items-center gap-[7px]">
              ...
              Lookback tiers
              <small className="normal-case tracking-normal text-text-tertiary font-normal ml-auto">
            ...
            <NumberField
              ...
              value={form.lookback_min_days}
              onChange={(v) => set('lookback_min_days', v)}
              min={1}
            />
            <NumberField
              label="Preferred"
              value={form.lookback_preferred_days}
              onChange={(v) => set('lookback_preferred_days', v)}
              min={1}
            />
            <NumberField
              label="Elite designation"
              value={form.lookback_elite_days}
              onChange={(v) => set('lookback_elite_days', v)}
              min={1}
            />
          </div>
          ...
          <div className="bg-bg-surface border border-border-subtle rounded-lg p-4">
            ...
            <NumberField
              label="Below threshold → yellow flag"
              value={form.strike_months_to_yellow}
              onChange={(v) => set('strike_months_to_yellow', v)}
              min={1}
            />
            <NumberField
              label="Consecutive below → disqualify"
              value={form.strike_months_to_disqualify}
              onChange={(v) => set('strike_months_to_disqualify', v)}
              min={1}
            />
            <NumberField
              label="Consecutive above → reinstate"
              value={form.strike_months_to_reinstate}
              onChange={(v) => set('strike_months_to_reinstate', v)}
              min={1}
            />
          </div>
        </div>
      ...
      <div className="bg-bg-surface border border-border-subtle rounded-lg p-4">
        ...
        <div
          className={`flex items-center gap-[9px] mt-3 px-3 py-[9px] rounded-md text-small ${weightsOk ? 'bg-pos-muted text-pos-text' : 'bg-neg-muted text-neg-text'}`}
        >
          {weightsOk ? <Check size={14} /> : <AlertTriangle size={14} />}
          <span>
            {weightsOk ? 'Weights sum correctly' : 'Weights must sum to 100 — adjust before saving'}
          </span>
      ...
      <div className="flex items-center gap-[9px] pt-[14px] border-t border-border-subtle">
        {gitStatus && (
          <div
            className={`flex items-center gap-[7px] text-micro px-[11px] py-[5px] rounded-pill ${gitStatus.is_dirty ? 'bg-warn-muted text-warn-text' : 'bg-pos-muted text-pos-text'}`}
          >
            <GitBranch size={12} />
        ...
        {gitStatus?.last_commit_message && (
          <span className="text-micro text-text-tertiary">
            last commit · "{gitStatus.last_commit_message}" ·{' '}
            {gitStatus.last_commit_at
              ? new Date(gitStatus.last_commit_at).toLocaleDateString()
              : ''}
          </span>
        )}
      ...
      <div className="text-micro text-text-tertiary bg-bg-sunken border border-border-subtle rounded-md px-3 py-[10px] flex gap-2 items-start">
        ...
        <span>
          <b className="text-text-secondary">
            Save writes the pipeline config file locally only — it does not commit or push.
          </b>{' '}
          The pipeline reads this exact file, so the UI and pipeline never disagree. Committing is a
          separate, deliberate step with a real message. Weights must sum to 100 before save is
          allowed.
        </span>
      </div>
    </div>
