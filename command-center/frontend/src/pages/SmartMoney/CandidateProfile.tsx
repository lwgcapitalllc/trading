...
export function CandidateProfile({
  ...
  const winRateData = candidate.monthly_win_rate.map((m) => ({
    month: m.month,
    value: m.value * 100,
  }))
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-[6px] text-small text-text-secondary hover:text-text-primary transition-colors mb-2"
      >
        <ArrowLeft size={14} />
        Back to rankings
      </button>
      ...
      <div className="bg-bg-surface border border-border-subtle rounded-lg p-4">
        ...
        <div className="flex items-center gap-[14px] pb-4 mb-4 border-b border-border-subtle">
          <div
            className="w-[46px] h-[46px] rounded-[11px] flex-shrink-0 flex items-center justify-center font-bold text-[15px] text-white"
          ...
          <div>
            <div className="font-mono text-[13px] font-medium">
              {candidate.id.length > 20
                ? `${candidate.id.slice(0, 10)}…${candidate.id.slice(-6)}`
                : candidate.id}
            </div>
            <div className="text-micro text-text-secondary mt-[2px] flex gap-2 items-center">
              <span
                className={`inline-flex text-[10px] font-semibold px-2 py-[3px] rounded-pill uppercase tracking-[0.4px] ${candidate.market === 'crypto' ? 'bg-accent-muted text-accent-text' : 'bg-gold-muted text-gold-text'}`}
              >
                {candidate.market}
              </span>
              ...
              {candidate.yellow_flag_count > 0 && (
                <span className="inline-flex text-[10px] font-semibold px-2 py-[3px] rounded-pill bg-warn-muted text-warn-text">
                  ⚑ {candidate.yellow_flag_count} yellow flag
                  {candidate.yellow_flag_count > 1 ? 's' : ''}
                </span>
              )}
            </div>
          </div>
          <div className="ml-auto text-right">
            <div className="text-[30px] font-bold tracking-[-1px] text-gold-text mono">
              {candidate.composite_score.toFixed(0)}
            </div>
            <div className="text-[10px] text-text-tertiary uppercase tracking-[0.7px]">
              Composite · rank {candidate.rank}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-5 gap-[10px] mb-[18px]">
          <StatCard
            label="Acct value"
            value={candidate.account_value != null ? fmtMoney(candidate.account_value) : '—'}
          />
          <StatCard
            label="All-time PnL"
            value={
              candidate.all_time_pnl != null
                ? fmtMoney(candidate.all_time_pnl, candidate.all_time_pnl >= 0 ? '+' : '-')
                : '—'
            }
            subVariant={
              candidate.all_time_pnl != null
                ? candidate.all_time_pnl >= 0
                  ? 'pos'
                  : 'neg'
                : 'neutral'
            }
          ...
          <StatCard
            label="Peak DD"
            value={`${candidate.peak_drawdown.toFixed(1)}%`}
            subVariant="neg"
          />
            ...
            <div className="space-y-[9px]">
              {Object.entries(candidate.score_breakdown).map(([key, val]) => (
                <div key={key} className="flex items-center gap-[10px] text-micro">
                  <span className="w-[135px] flex-shrink-0 text-text-secondary">
            ...
            <table className="w-full text-micro">
              <tbody>
                ...
                <tr>
                  ...
                  <td className="text-right py-[5px]">
                    {candidate.typical_entry_hour_utc != null
                      ? `${candidate.typical_entry_hour_utc}:00 UTC`
                      : '—'}
                  </td>
                </tr>
                <tr>
                  ...
                  <td className="text-right py-[5px]">
                    {candidate.avg_hold_time_hours != null
                      ? `${candidate.avg_hold_time_hours.toFixed(1)} hours`
                      : '—'}
                  </td>
                </tr>
