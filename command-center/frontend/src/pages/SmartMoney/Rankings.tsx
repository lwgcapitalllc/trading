...
export function Rankings({ candidates, onSelect }: RankingsProps) {
  ...
  const SortTh = ({ col, label, right }: { col: keyof Candidate; label: string; right?: boolean }) => (
    <th
  ...
  return (
    <div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              <th className="text-left text-[10px] font-semibold uppercase tracking-[0.7px] text-text-tertiary px-[14px] py-[10px] bg-bg-surface-2 border-b border-border-subtle">Wallet / account</th>
          ...
          <tbody>
            {filtered.map(c => (
              ...
              >
                <td className={`px-[14px] py-[11px] font-semibold w-[34px] ${c.rank <= 5 ? 'text-gold-text' : 'text-text-secondary'}`}>
                  {c.rank <= 5 ? <Star size={12} className="inline mr-1 fill-gold-text text-gold-text" /> : null}
                  {c.rank}
                </td>
                ...
                <td className="px-[14px] py-[11px]">
                  <span className={`inline-flex items-center text-[10px] font-semibold px-2 py-[3px] rounded-pill uppercase tracking-[0.4px] ${c.market === 'crypto' ? 'bg-accent-muted text-accent-text' : 'bg-gold-muted text-gold-text'}`}>
                    {c.market}
                  </span>
                </td>
                ...
                <td className="px-[14px] py-[11px]">
                  <div className="flex items-center gap-[9px]">
                    <span className="font-semibold w-[26px] mono">{c.composite_score.toFixed(0)}</span>
                    <div className="flex-1 h-[5px] bg-bg-surface-2 rounded-pill overflow-hidden min-w-[50px]">
                      <div className="h-full rounded-pill" style={{ width: `${c.composite_score}%`, background: 'linear-gradient(90deg, #2dd4bf, #d9a441)' }} />
                    </div>
                  </div>
                </td>
      ...
      <div className="mt-3 text-micro text-text-tertiary bg-bg-sunken border border-border-subtle rounded-md px-3 py-[10px] flex gap-2 items-start">
        ...
        <span>Click any row to open the full candidate profile. Top 5 shortlist marked in gold. Sort on any numeric column.</span>
      </div>
    </div>
  )
}
