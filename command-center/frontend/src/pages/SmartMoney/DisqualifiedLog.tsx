...
function categorizeReason(reason: string): ReasonCategory {
  ...
  if (r.startsWith('api error') || r.includes('all 3 attempts') || r.includes('api error')) return 'api_error'
  if (r.includes('drawdown')) return 'drawdown'
  if (r.includes('single trade') || r.includes('concentrat') || r.includes('pnl share')) return 'concentration'
  if (
    r.includes('win rate') || r.includes('strike') ||
    r.includes('yellow') || r.includes('red flag')
  ) return 'win_rate'
  if (
...
function formatReason(reason: string): string {
  ...
  return reason.replace(/Window starting (\d{13}) has only (\d+) active weeks \(need (\d+)\)/, (_, ts, actual, needed) => {
    const date = new Date(parseInt(ts)).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
...
type FilterKey = 'All reasons' | 'Win rate' | 'Activity' | 'Drawdown' | 'Concentration' | 'API errors'

const REASON_FILTERS: FilterKey[] = ['All reasons', 'Win rate', 'Activity', 'Drawdown', 'Concentration', 'API errors']
...
export function DisqualifiedLog({ disqualified }: { disqualified: DisqualifiedCandidate[] }) {
  ...
  const counts: Record<FilterKey, number> = {
    ...
    'Concentration': disqualified.filter(d => categorizeReason(d.reason) === 'concentration').length,
    ...
    <div>
        ...
        <div className="flex bg-bg-surface border border-border-subtle rounded-md overflow-hidden">
          {REASON_FILTERS.map(f => (
            ...
            >
              {f}
              {counts[f] > 0 && (
                <span className={`text-[9px] font-semibold tabular-nums ${reasonFilter === f ? 'text-accent' : 'text-text-tertiary'}`}>
                  {counts[f]}
                </span>
              )}
            </span>
      ...
      <div className="bg-bg-surface border border-border-subtle rounded-lg overflow-hidden">
        <table className="w-full border-collapse">
          <thead>
            <tr>
              {['Wallet / account', 'Market', 'Source', 'Stage', 'Disqualification reason'].map(h => (
          ...
          <tbody>
            {filtered.map((d, i) => {
              ...
              return (
                <tr key={i} className="border-b border-border-subtle last:border-0 hover:bg-bg-hover/40 transition-colors duration-[80ms]">
                  ...
                  <td className="px-[14px] py-[11px] font-mono text-[11px] text-text-tertiary">{d.stage}</td>

                  {/* Reason: category badge + truncated reason text */}
                  <td className="px-[14px] py-[11px] min-w-0">
                    <div className="flex flex-col gap-[4px]">
                      <span className={`inline-flex self-start text-[10px] font-semibold px-[8px] py-[2px] rounded-pill uppercase tracking-[0.4px] ${CATEGORY_BADGE[cat]}`}>
                        {CATEGORY_LABELS[cat]}
                      </span>
                      <span
      ...
      <div className="mt-3 text-micro text-text-tertiary bg-bg-sunken border border-border-subtle rounded-md px-3 py-[10px] flex gap-2 items-start">
        ...
        <span>Nothing is silently dropped — every removed candidate is logged with its reason and the stage that removed it. Hover any reason for the full text.</span>
      </div>
    </div>
  )
}
