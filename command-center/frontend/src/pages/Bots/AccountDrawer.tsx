...
import { AccountForm, nameOf, type SettingsFocus } from './AccountForm'
...
import { BackButton, Change, SectionTitle } from './drawerParts'
...
export function AccountDrawer({
  ...
  // Which card the settings open on, when a missing step in the heading was clicked. `at` makes a
  // second click on the same step scroll to it again.
  const [focus, setFocus] = useState<{ to: SettingsFocus; at: number } | null>(null)
  const openSettings = (to?: SettingsFocus) => {
    setFocus(to ? { to, at: Date.now() } : null)
    setEditing(true)
  }
    ...
    reg && !reg.assignable
      ...
      : reg?.channels_reason
        ...
        : reg?.has_password === false
          ? 'No password is stored for this account, so a bot put here cannot log in. Add the trading password first (Settings).'
          : null
  ...
  // The login leads: it is what the broker, the terminal, every config and every refusal names this
  // account by; the label is a nickname somebody typed here.
  const title = (
  ...
  // 🔴 THE READINESS FACTS. Each answers *why did that move fail* BEFORE somebody makes it — the only
  // moment the answer is worth anything. Since 2026-09-13 they are a FIXED list of four steps
  // (`readiness.tsx`), so a missing step reads differently from a done one; a missing step is a
  // button that opens the settings on its fix.
  const subtitle = (
    <div className="flex flex-col gap-[8px] mt-[5px]">
      ...
      {reg && <ReadinessStrip reg={reg} onFix={openSettings} />}
    </div>
  )
  const shell = (body: ReactNode, drawerFooter?: ReactNode, actions?: ReactNode) => (
    <Drawer
      open
      onClose={onClose}
  ...
  // ⚠ The settings are a STEP of this panel under the same heading, so the checklist stays in view
  // while its missing steps are filled in — and the form's Save row takes the pinned footer.
  if (editing && reg) {
    return (
      <AccountForm
        existing={reg}
