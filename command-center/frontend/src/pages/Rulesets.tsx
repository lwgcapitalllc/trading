...
function ladderLines(s: ScalingLadder): string[] {
  ...
  if (s.tiers?.length) {
    ...
    lines.push(
      ...s.tiers.map(
        (t) => `+$${(t.profit_trigger ?? 0).toLocaleString()} profit: ${t.mini}/${t.micro}`
      )
    )
    return lines
  }
    ...
    <div className="relative group inline-block">
      ...
      <div className="absolute right-0 top-full mt-1 z-20 hidden group-hover:block bg-bg-sunken border border-border-default rounded-md px-3 py-2 shadow-xl whitespace-nowrap">
        <div className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wide mb-1">
          Minis + micros share one cap
        </div>
        <div className="text-[11px] font-mono text-text-secondary">
          1 mini = {ratio} micros against the limit
        </div>
        <div className="text-[11px] text-text-tertiary mt-0.5">
          profit on excess contracts is voided, not a breach
        </div>
      </div>
    </div>
  )
...
function ContractsCell({ maxContracts }: { maxContracts: Record<string, unknown> | null }) {
  ...
  const fixedLabel =
    mc.mini_max != null
      ? `${mc.mini_max} mini / ${mc.micro_max ?? '—'} micro`
      : mc.any != null
        ? `${mc.any} contracts`
        : null
    ...
    <div className="relative group/scale inline-block">
      ...
      {lines.length > 0 && (
        <div className="absolute right-0 top-full mt-1 z-20 hidden group-hover/scale:block bg-bg-sunken border border-border-default rounded-md px-3 py-2 shadow-xl whitespace-nowrap">
          <div className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wide mb-1">
            {ratchet
              ? 'Scaling ladder — retained once reached'
              : 'Scaling bands — can move both ways'}
          </div>
          {lines.map((l, i) => (
...
function PersonalRulesEditModal({ ruleset, onClose }: { ruleset: Ruleset; onClose: () => void }) {
  ...
  const inputCls =
    'bg-bg-sunken border border-border-subtle rounded px-2.5 py-[5px] text-[12px] text-text-primary w-full focus:outline-none focus:border-accent transition-colors'
  ...
  function handleSave() {
    ...
    const fields: Array<[keyof typeof form, string, (v: number) => boolean]> = [
      ...
      [
        ...
        'Max loss days must be at least 1',
      ...
      <div className="bg-bg-surface border border-border-default rounded-xl w-full max-w-[440px] flex flex-col shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={set('daily_loss_cap')}
                placeholder="500"
              />
            </div>
            <div>
              <label className={labelCls}>Daily Profit Target ($)</label>
              <input
                type="number"
                step="50"
                min="0"
                ...
                onChange={set('daily_profit_target')}
                placeholder="1000"
              />
            </div>
            <div>
              <label className={labelCls}>Max DD from Peak (%)</label>
              <input
                type="number"
                step="0.5"
                min="0"
                max="100"
                className={inputCls}
                value={form.max_drawdown_from_peak_pct}
                onChange={set('max_drawdown_from_peak_pct')}
                placeholder="15"
              />
            </div>
            <div>
              <label className={labelCls}>Max Consecutive Loss Days</label>
              <input
                type="number"
                step="1"
                min="1"
                className={inputCls}
                value={form.max_consecutive_loss_days}
                onChange={set('max_consecutive_loss_days')}
                placeholder="3"
              />
            </div>
          </div>
          <p className="text-[11px] text-text-tertiary leading-relaxed">
            A day losing the cap counts toward the consecutive-day fail; hitting the profit target
            halts the day (informational). Drawdown is measured from the equity peak. Re-evaluate
            runs after changing rules — existing verdicts keep the values they were graded with.
          </p>
        </div>
        <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-border-subtle">
          ...
          <button
            ...
            className="px-4 py-[7px] rounded-md text-[13px] font-medium bg-accent text-bg-base hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
          >
...
function RulesetsView() {
  ...
  const propRulesets =
    rulesets?.filter((r) => r.ruleset_type === 'prop_eval' || r.ruleset_type === 'prop_funded') ??
    []
  const otherRulesets =
    rulesets?.filter((r) => r.ruleset_type !== 'prop_eval' && r.ruleset_type !== 'prop_funded') ??
    []
  ...
  const showPersonal =
    (brandFilter === null || brandFilter === PERSONAL) && otherRulesets.length > 0
  ...
  return (
    <div className="space-y-6">
      <StickyHeader>
        {(scrolled) => (
          <div
            className={`flex items-center justify-between gap-3 transition-all duration-200 ${scrolled ? 'pb-2.5' : ''}`}
          >
            <h1
            ...
            {hasData && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={filterBtnCls(brandFilter === null)}
                >
                  All
                </button>
                {brands.map((b) => (
                  <button
                    ...
                    className={filterBtnCls(brandFilter === b)}
                  >
                    {b}
                  </button>
                ))}
                {otherRulesets.length > 0 && (
                  <button
                    onClick={() => setBrandFilter(brandFilter === PERSONAL ? null : PERSONAL)}
                    className={filterBtnCls(brandFilter === PERSONAL)}
                  >
                    {PERSONAL}
                  </button>
                )}
...
function RulesetRow({ ruleset, personal = false }: { ruleset: Ruleset; personal?: boolean }) {
  ...
  return (
    <>
      {editing && editable && (
        <PersonalRulesEditModal ruleset={ruleset} onClose={() => setEditing(false)} />
      )}
      <tr className="hover:bg-bg-hover transition-colors">
        <td className="px-4 py-3">
          <div className="flex items-center gap-1.5">
            ...
            {editable ? (
              ...
              <span
                title="Firm rules — not editable"
                className="text-text-tertiary ml-0.5 cursor-default"
              >
                <Lock size={10} />
              </span>
            )}
        ...
        {!personal ? (
          <>
            ...
            <td className="px-4 py-3 font-mono tabular-nums text-text-secondary">
              {ruleset.min_trading_days != null ? (
                <span title="Minimum trading days to pass the evaluation">
                  {ruleset.min_trading_days}
                </span>
              ) : (
                <span
                  className="text-text-tertiary"
                  title="No minimum trading days published for this challenge"
                >
                  —
                </span>
              )}
          ...
          <>
            ...
            <td className="px-4 py-3 font-mono tabular-nums text-text-secondary">
              {ruleset.max_consecutive_loss_days != null ? (
                ruleset.max_consecutive_loss_days
              ) : (
                <span className="text-text-tertiary">—</span>
              )}
            </td>
        ...
        <td className="px-4 py-3">
          <ContractsCell maxContracts={ruleset.max_contracts} />
        </td>
      </tr>
    </>
  )
