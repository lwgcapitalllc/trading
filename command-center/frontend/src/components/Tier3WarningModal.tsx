        ...
        <div className="flex-1 min-h-0 flex flex-col overflow-hidden px-5 py-4 gap-4">
          <div className="flex-shrink-0 space-y-4">
            <p className="text-[13px] text-text-secondary">
              This strategy scored <WorthinessBadge worthiness={run.worthiness} /> on{' '}
              <span className="font-mono text-text-primary">{run.instrument}</span>. Reason:{' '}
              <span className="text-warn-text">{reason}</span>.
            </p>
            <p className="text-[13px] text-text-secondary">
              Optimizing a Tier 3 strategy rarely changes the outcome. Before optimizing, consider
          ...
          {isLoading ? (
            ...
            (summary.instrument_results.length > 0 || summary.untested_instruments.length > 0) ? (
            <div className="flex-1 min-h-0 flex flex-col border border-border-subtle rounded-lg overflow-hidden">
                    ...
                    {summary.instrument_results.map((r) => (
                      <tr key={r.instrument} className="hover:bg-bg-hover">
                        ...
                        <td className="px-3 py-2">
                          <WorthinessBadge
                            worthiness={
                              r.best_worthiness
                                ? {
                                    tier: r.best_worthiness as
                                      'TIER_1_STRESS_TEST' | 'TIER_2_OPTIMIZE' | 'TIER_3_DISCARD',
                                    reason: null,
                        ...
                        <td className="px-3 py-2 text-text-tertiary">
                          {r.tested_at
                            ? new Date(r.tested_at).toLocaleDateString('en-US', {
                                month: 'short',
                                day: 'numeric',
                              })
                            : '—'}
                        ...
                        <td className="px-3 py-2 text-right">
                          {r.best_run_id && (
                            <button
                              onClick={() => {
                                onClose()
                                navigate(`/backtests/runs/${r.best_run_id}`)
                              }}
                      ...
                      <tr>
                        <td colSpan={4} className="p-0">
                          ...
                          >
                            ...
                            {showUntested ? 'Hide' : 'Show'} {summary.untested_instruments.length}{' '}
                            untested instrument
                            {summary.untested_instruments.length !== 1 ? 's' : ''}
                          </button>
                        </td>
                      </tr>
                    )}
              ...
              {summary.untested_instruments.length > 0 && (
                <div className="flex-shrink-0 px-3 py-3 border-t border-border-subtle">
                  ...
                  >
                    <Layers size={13} />
                    Run on {summary.untested_instruments.length} untested instrument
                    {summary.untested_instruments.length !== 1 ? 's' : ''}
                  </button>
                </div>
              )}
            ...
            <div className="flex-shrink-0 bg-bg-sunken border border-border-subtle rounded-lg px-4 py-3 text-[13px] text-text-secondary">
              This strategy has only been tested on {run.instrument}. Consider running it on other
            ...
            <div className="flex items-start gap-2 px-3 py-2.5 rounded-md bg-warn-muted/40 border border-warn-text/20 mb-3">
              ...
              <p className="text-[12px] text-warn-text leading-snug">
                <span className="font-semibold">
                  {RUNNER_LABEL[runnerScope(run.runner)]} is busy:
                </span>{' '}
