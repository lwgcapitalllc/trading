...
const ROWS: { tier: WorthinessScore['tier']; desc: string }[] = [
  { tier: 'TIER_1_STRESS_TEST', desc: 'Strong enough to stress test next. Profit factor above 1.3, drawdown safely under the firm limit, and at least 50 trades.' },
  { tier: 'TIER_2_OPTIMIZE',    desc: 'Promising but not there yet. Profit factor between 0.8 and 1.3, or drawdown nearing the limit — worth optimizing. At least 30 trades.' },
          ...
          {ROWS.map(r => (
            <div key={r.tier} className="flex items-start gap-3">
              <div className="w-[92px] flex-shrink-0 pt-[1px]">
                <WorthinessBadge worthiness={{ tier: r.tier, reason: null, computed_against_firm: null }} />
              </div>
