...
└── pages/
    ...
    ├── BacktestDetail.tsx    **Tune button carries a COUNT badge** of the iterations already run from this run (`source_run_id === runId`, off the unfiltered `useBacktestRuns()` so it shares the Runs list's cache entry) — clicking it opens the workbench where they all live. Without the badge the only way to discover a run had ever been tuned was to go back to the Runs list and spot the nested Tune rows. Full run detail — params side panel, per-firm evaluation + KPIs, tabbed charts, logs, News & Holiday filter accordion (inline `NewsFilterCard`, driven by the page's `useNewsFilter` hook — which also feeds the Equity chart)
...
| News & Holiday filter | ✅ Live (NT8 + Python) | Post-run **accordion** on BacktestDetail (`NewsFilterCard`, inline; collapsed by default, state in `localStorage` `bt_news_panel`). State lives in the page-level **`useNewsFilter`** hook, not in the card, because two things read it: the card, and **the main Equity chart, which redraws on the kept trades** (`equityCurve = news.filteredCurve ?? run.equity_curve`). `useRunNews` tags the RAW `equity_curve` trades; News Included/Removed toggle + before/after sliders (default 15/30) → the 5 mini-KPIs with deltas vs the raw run (`newsKpisFrom`). Bank holidays always excluded (off the toggle). Toggle start = the strategy's `avoid_news` (`removeNews = choice ?? avoidNews`). Coverage-honest (untagged where no data; the pre-`entry_ms` note offers "Reload charts" on NT8 only — it re-exports from the VPS and cannot help MT5/Python). **Forex/MT5 not wired — TODO #3** (needs MT5 `entry_ms` + non-UTC broker timezone handling) |
