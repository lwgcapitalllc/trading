...
button did not exist — on the one banner a reader is looking at because something failed. It is
wired to `RerunModal` for a standalone run and to the direct re-fire for a sweep child or optimizer
combo, and it is disabled while that platform holds a job (the backend 409s, and a button whose
only outcome is an error toast is not a button).
