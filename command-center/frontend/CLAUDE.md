...
└── pages/
    ...
    ├── Optimizations.tsx     OWN top-level RESEARCH page (route /optimizations) — optimization list table. Decoupled from the Backtests tab. Row count by the title; checkbox multi-select bulk delete (matches the Backtests pattern: select-all header checkbox + per-row, "Delete N" button by the count, `ConfirmDeleteModal`, `Promise.allSettled` over `DELETE /optimizations/{id}`). Running optimizations are not selectable (cancel first)
    ...
    ├── StressTests.tsx       stress test list — grade badge, strategy/instrument/status columns, prob breach/pass, created. Row count by the title; a collapsible `GradeLegend` above the table; checkbox multi-select bulk delete (matches the Backtests pattern: select-all header checkbox + per-row, "Delete N" button by the count, `ConfirmDeleteModal`, `Promise.allSettled` over `DELETE /stress-tests/{id}`)
