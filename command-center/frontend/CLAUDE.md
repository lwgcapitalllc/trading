...
The Equity chart is a TradingView-style panel: the cumulative-PnL line is **colour-split at the starting balance** (green above, red below — `startEq = data[0].equity - data[0].profit`, offset mapped to the fill bbox so the flip lands on the break-even line), the Y axis is tick-anchored on `startEq` (starting balance always labelled), a dot on every trade point (hover → Balance + Favorable/Adverse excursion), one opt-in **Histogram** `SeriesToggle`, and regime bands that skip UNKNOWN (chart matches the legend). Excursion needs `favorable`/`adverse` on `models.EquityPoint` (else FastAPI drops them). Details in `FRONTEND_BUILD_NOTES.md`.
