...
The Equity chart is a TradingView-style panel: the cumulative-PnL line is **colour-split at the starting balance** (green above, red below — `startEq = data[0].equity - data[0].profit`, so the flip lands on the break-even line), a dot on every trade point (hover → Cumulative P&L + Favorable/Adverse excursion), and two opt-in `SeriesToggle` pills on the equity tab — **Histogram** (per-trade profit bars) and **Excursions** (favorable/adverse per trade — shown only when the run carries the data; Python runner only). Details in `FRONTEND_BUILD_NOTES.md`.
