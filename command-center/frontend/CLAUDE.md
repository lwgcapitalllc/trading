...
The Equity chart is a TradingView-style panel: the cumulative-PnL line is **colour-split at break-even** (green above the starting equity, red below, via `linearGradient`s that hard-stop at the start-equity offset), with three opt-in `SeriesToggle` pills on the equity tab — **Histogram** (per-trade profit bars), **Excursions** (favorable/adverse per trade — shown only when the run carries the data; Python runner only), **Run-ups & drawdowns** (green/red period bands from the equity curve). The excursion tooltip and toggles are documented in `FRONTEND_BUILD_NOTES.md`.
