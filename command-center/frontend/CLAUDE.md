...
│   ├── MonteCarloFan.tsx    equity path fan (100 paths, p10–p90) — shared `BANDS` array drives the lines, the percentile-named tooltip, AND the Luckier→Unluckier key below the chart; axes labelled (Cumulative P&L / Trade #)
...
└── pages/
    ...
    ├── StressTestDetail.tsx  stress test detail — grade column card (coloured strip + name + ruleset chip + reasons), source backtest card (links back to run via useBacktestRun), MetricCard MC stats with pos/neg colouring, InfoTip tooltips, prob bars, fan chart, drawdown dist, walk-forward, sensitivity. Charts are grouped into three labelled `AnalysisSection`s (Monte Carlo / Walk-Forward / Sensitivity), each with a one-line "what this answers" — the MC group makes clear its stats+probs+fan+drawdown are one simulation viewed four ways. Section-header right-side metrics use `gradedRight()` → "{n}% · robust/acceptable/fragile" coloured by the grading thresholds (WF degradation 20/30, sensitivity 25/40), with "n/a · IS Sharpe ≤ 0" when not assessable. The dollar drawdown limit shown on cards and threaded to the charts is `ddLimit` — mirrors backend `effective_dd_limit_usd` (personal/demo = account_size × %-from-peak; prop = max_loss_eod), so personal rows never render a $0 limit. Pass-probability bar label is ruleset-aware (personal: "staying under drawdown limit")
...
- Charts: a **tabbed primary chart** (Equity / Price / Drawdown, ~520px, one at a time — the Price tab lazy-loads its ChartSpec via `PriceChartPanel`), a compact secondary **rail** (Daily P&L ~180px + Long/Short donuts), and a permanent **Performance by Regime** table. Regime context shows as **faint background bands** on the equity view (`ReferenceArea`, fillOpacity 0.1 — NOT line-segment colors), toggled by `RegimeOverlayToggle`; same treatment as the tune page.
- KPIs (`KpiGrid`): a flat grid — **6 core cards always shown** (Net P&L · Sharpe · Win Rate · Max DD % · Profit Factor · Calmar) plus **6 "more" cards** (Profit Concentration · Expectancy · Z-Score · Avg Trade · Worst Day · Worst Streak) revealed by a "More metrics" toggle (`showMoreKpis`, lifted to the page). Big-number cards with a sentiment-coloured left accent (`KPI_TONE_BORDER`, derived from the value's text colour). On `lg` the grid is **pinned to the evaluation card's measured height** (a `ResizeObserver` on the eval column → `fixedHeight`): collapsed = one tall row, expanded = two half-height rows summing to the SAME total (each card gets an explicit px height so the shrink animates; value font 44px→28px). Trade Count is NOT a card — it's the standout at the bottom of the eval card (`TradeCountStandout`); the single-eval case hides the eval-card name (it's in the breadcrumb), multi-eval keeps it.
