...
**Last reviewed:** 2026-07-30 — the News & Holiday filter panel was **redesigned to sit in the same view as the Equity chart it reshapes**: knobs left / resulting numbers right on ONE flex-wrap row, and the prose cut to chips + a single `InfoTip` (expanded height ~256px → ~100px). No behaviour changed — same hook, same filter, same KPIs. See `## The News & Holiday filter panel` below for the rules that keep it short. Earlier: 2026-07-29 — the price chart's **fib levels are configurable** (add / remove / retune / recolour / hide, per drawing or as the tool's persisted default), and the News & Holiday filter became a collapsed-by-default accordion whose state lives in a page-level `useNewsFilter` hook, so the MAIN Equity chart redraws on the kept trades (its own duplicate mini-curve is gone); 2026-07-28 — price chart: a **Go to date** pill that jumps the view to a typed date (paging history in on the way); earlier, the Analysis dropdown (Trades + Winners/Losers, Blocked and **Missed** + per-reason filters), and it now ships/opens on the run's own timeframe with older history paged in on scroll-left
...
└── pages/
    ...
    ├── BacktestDetail.tsx    **Tune button carries a COUNT badge** of the iterations already run from this run (`source_run_id === runId`, off the unfiltered `useBacktestRuns()` so it shares the Runs list's cache entry) — clicking it opens the workbench where they all live. Without the badge the only way to discover a run had ever been tuned was to go back to the Runs list and spot the nested Tune rows. Full run detail — params side panel, per-firm evaluation + KPIs, tabbed charts, logs, News & Holiday filter accordion (inline `NewsFilterCard` + `NewsMiniKpi`/`NewsCountChip`, driven by the page's `useNewsFilter` hook — which also feeds the Equity chart)
...
## The News & Holiday filter panel — why it is terse

**Redesigned 2026-07-30. Read this before adding anything to `NewsFilterCard`.**

The panel's constraint is not "look tidy" — it is that **it must fit on screen at the same time as the Equity chart it reshapes.** It is the only control on the page whose effect lands on a chart somewhere else, so a knob you cannot see the effect of is worse than no knob. Expanded, it went from ~256px to ~100px and stayed there on purpose. Two rules follow.

**1. Layout is an instrument row, not a stack.** Knobs LEFT (toggle + both window sliders on line 1, the excluded-trade count chips on line 2), resulting numbers RIGHT (the five mini-KPIs). One `flex flex-wrap`; the KPI block carries `flex-1 min-w-[680px]` so that when the params side panel squeezes the card, the numbers drop to their own full-width line instead of shrinking. **Use flex-wrap, never a viewport breakpoint** — the side panel changes the width available *inside* this card, which a `lg:`/`xl:` prefix cannot see. Uneven KPI columns (`lg:grid-cols-[1.35fr_1fr_1fr_1.25fr_0.85fr]`) for the same reason `KPI_COLS` is uneven above: the money tiles carry the long values, and a long value is solved by ROOM, never by shrinking that one tile's type. Both money tiles use `FitMoney` as the last-resort fallback.

**2. Rules live in the `InfoTip`, not on screen.** Anything that is true every time you open the panel — bank holidays are always excluded, the toggle only governs news-window trades, only the Equity chart follows the filter — is read once and then never again, so it costs height for nothing. It goes in the panel's single shared `InfoTip`. What stays on screen is only what CHANGES: the toggle state, the two window values, the two counts, the five numbers. Concretely: counts are `NewsCountChip` (coloured dot + number + noun), not sentences; the KPI tiles are borderless (the card is already a box, and nested boxes read heavy); and the card has **no title of its own** — the `SectionLabel` directly above it is the title, so the header carries only live state (`139 of 142 trades kept`) plus an accent "Equity chart filtered" chip when `active`.

If you need to add a control here, take a line out first.
...
| News & Holiday filter | ✅ Live (NT8 + Python) | Post-run **accordion** on BacktestDetail (`NewsFilterCard`, inline; collapsed by default, state in `localStorage` `bt_news_panel`). State lives in the page-level **`useNewsFilter`** hook, not in the card, because two things read it: the card, and **the main Equity chart, which redraws on the kept trades** (`equityCurve = news.filteredCurve ?? run.equity_curve`). `useRunNews` tags the RAW `equity_curve` trades; a News Keep/Remove toggle + before/after sliders (default 15/30) → the 5 mini-KPIs with deltas vs the raw run (`newsKpisFrom`). Bank holidays always excluded (off the toggle). **Laid out knobs-left / numbers-right on one flex-wrap row and deliberately terse — see `## The News & Holiday filter panel` for the rules that keep it short enough to share a view with the Equity chart.** Toggle start = the strategy's `avoid_news` (`removeNews = choice ?? avoidNews`). Coverage-honest (untagged where no data; the pre-`entry_ms` note offers "Reload charts" on NT8 only — it re-exports from the VPS and cannot help MT5/Python). **Forex/MT5 not wired — TODO #3** (needs MT5 `entry_ms` + non-UTC broker timezone handling) |
