...
- KPIs (`KpiGrid`): a **data-driven** grid (a `groups` array, not repeated card markup) of **13 metric cards in 5 labelled group panels** (`lg:grid-cols-5`, equal-height cards, panels stretch to equal height): **Outcome** (Net P&L · Max DD % of Capital · Profit Concentration) · **Risk-Adjusted Quality** (Sharpe · Calmar · Profit Factor) · **Trade Behavior** (Win Rate · Expectancy · Z-Score) · **Activity** (Trade Count · Avg Trade) · **Tail Risk** (Worst Day · Worst Streak). Panels share ONE neutral accent (`border-accent` + faint `bg-accent/[0.04]`) — no per-group hue, since red/amber/violet/cyan all carry meaning (value colours, warnings, the regime palette). Cards are rendered inline (the old `MetricCard` was removed).
...
- **Account-balance what-if slider is docked on the Max DD % of Capital card** (not the header) — it only rebases *that* %. `KpiGrid` takes `balance` / `defaultBalance` / `onBalanceChange`. The % uses a **trade-derived** drawdown rebased to the chosen balance (`rebaseEquity` + `maxDrawdownOf`), so it's identical across NT8/MT5. **Calmar is capital-independent by design** (CAGR÷MaxDD% — the balance cancels) and does NOT move with the slider; every other metric comes from the trades, not the balance.
