...
| MT5 backtest detail | ✅ Live | `BacktestDetail` branches on `run.runner === 'mt5'` (sourced from `runner` field on API response): `MT5_RUN_STEPS` (Launch→Testing→Parse) passed to `RunningBanner`; `getFailureGuidance()` returns runner-specific message; "Load chart data from NT8", Refresh chart, and Stress Test buttons hidden; chart empty-state copy is MT5-specific; daily P&L empty-state copy platform-agnostic. |
