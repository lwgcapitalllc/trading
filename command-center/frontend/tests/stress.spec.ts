...
test('a compounding run shows its drawdown in PERCENT, the unit the grade read', async ({ page }) => {
  ...
  await openMutated(page, d => {
    d.dd_basis = 'percent'
    d.pct1_max_dd = 43_444; d.pct1_max_dd_pct = 62.1
    d.pct5_max_dd = 31_000; d.pct5_max_dd_pct = 53.2
    d.median_max_dd = 24_675; d.median_max_dd_pct = 31.0
    d.status = 'complete'
  })
...
test('a fixed-size run still shows dollars', async ({ page }) => {
  await openMutated(page, d => {
    d.dd_basis = 'dollars'
    d.pct1_max_dd_pct = null; d.pct5_max_dd_pct = null; d.median_max_dd_pct = null
...
test('the grade reasons are rendered — they were computed, stored, and shown nowhere', async ({ page }) => {
  await openMutated(page, d => {
    d.status = 'complete'
    d.grade = null
    d.grade_reasons = [
...
test('the out-of-sample trade counts are on screen — the whole verdict turns on them', async ({ page }) => {
  ...
  await openMutated(page, d => {
    ...
    d.walk_forward_summary = [1, 2, 3, 4, 5].map(w => ({
      window: w, is_pnl: 100, oos_pnl: 50, is_sharpe: 1.5, oos_sharpe: 0.5,
      is_trades: 15, oos_trades: 6, is_pf: null, oos_pf: null,
    }))
...
test('the native path reports PROFIT FACTOR, not a row of zero Sharpes', async ({ page }) => {
  ...
  await openMutated(page, d => {
    ...
    d.walk_forward_summary = [1, 2, 3].map(w => ({
      window: w, is_pnl: 100, oos_pnl: 50, is_sharpe: null, oos_sharpe: null,
      is_trades: null, oos_trades: null, is_pf: 1.8, oos_pf: 1.2,
    }))
  })
  await page.getByRole('button', { name: 'Walk-Forward', exact: true }).click()
...
test('a shift that IMPROVED the result is not drawn as a loss', async ({ page }) => {
  ...
  await openMutated(page, d => {
    ...
    d.sensitivity_summary = {
      aplus_window: {
        '+10%': { degradation: 0.31, pf_delta_pct: 31.0, profit_factor: 5.4, run_id: 'a', new_value: 4752 },
        '-10%': { degradation: 0.12, pf_delta_pct: -12.0, profit_factor: 3.6, run_id: 'b', new_value: 3888 },
      },
    }
  ...
  await openMutated(page, d => {
    ...
    d.sensitivity_summary = {
      only_param: {
        ...
        '-10%': { degradation: 0.2, pf_delta_pct: -20, profit_factor: 3.0, run_id: 'b', new_value: 9 },
      },
    }
...
test('the equity fan carries no drawdown-limit line', async ({ page }) => {
  ...
  const fan = page.locator('div').filter({ hasText: /^Equity Path Fan$/ }).locator('..')
