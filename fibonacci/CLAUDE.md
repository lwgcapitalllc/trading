...
events matched). Sniper fib and Macro fib not yet ported.
...
```
fibonacci/
├── geometry.py     ← the one shared fib-math core (fib_level / fib_levels / origin_index)
├── engine.py       ← the per-fib state machines (StructureFib now; SniperFib, MacroFib next)
├── types.py        ← StructureSnapshot (input), FibTouch / StructureFibEvents (output)
...
Only **Structure** is implemented so far. Its gating logic (ported exactly):
0.618 (E1) is the gate — it must be reached before anything else arms; the deeper retrace levels
(E2/E3/E4/1.0) only register while price is at/through 0.618; the targets (TP1–TP5) only register
from the bar **after** 0.618 was first reached; a new leg (origin bar changes) resets every touch.
...
Same stateful-streaming rationale as `market_structure/` (see its CLAUDE.md): the touch/gate state
carries bar-to-bar and cannot be recomputed from a single bar. Build one `StructureFib` per
symbol/timeframe, feed one closed bar per `update()`.
...
## Validation (Pine ↔ Python parity) — Structure fib GREEN

**Result (2026-07-02):** full parity for the Structure fib on a `VANTAGE_XAUUSD, 15m` export
...
the Python engines start cold — the same cold-start pattern as `market_structure/`. Re-run
`compare_fib.py` after any change to `StructureFib` or the fib block in `mpc_assistant.pine`.
...
1. `indicators/fib_export.pine` — the external structure engine (byte-identical to
   `structure_engine_export.pine`) + the real Structure fib lifted from `mpc_assistant.pine`
   (compute + touched machine, drawing removed) + `plot()` columns for the fib outputs
   (`px_fib_<lvl>_price`, `px_fib_<lvl>_touch`, `px_fib_active/dir/origin`). Put it on a chart,
   export chart data to CSV, drop it in `fibonacci/exports/` (git-ignored).
2. `fibonacci/tools/compare_fib.py <that.csv>` — runs the REAL pipeline (StructureEngine →
   StructureSnapshot → StructureFib) on the CSV's candles and diffs against the `px_fib_*`
   columns, bar by bar. Exit 0 = parity. Standard library only. `--warmup N` skips cold-start
   bars (the tool prints the last mismatching bar to help pick N).

The tool's plumbing is smoke-tested (synthetic pass + injected-mismatch detection). What remains
is running it on a real TradingView export. Until that is green on real candles, status stays
"under construction" and no live consumer should trade off it. Expect early-bar mismatches to be
warmup (structure not yet converged, or a leg that began before the export window) — parity should
hold from the first full in-window leg onward.

## References

- Pine source of truth: `indicators/mpc_assistant.pine` (fib blocks `GRP_FIBO` ~2009-2114,
  `GRP_SNIPER` ~2512, `GRP_MACRO` ~2290+) and its live "MPC - JARVIS" confirmation table, which
  defines the event model this engine reproduces.
