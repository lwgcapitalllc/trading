...
- **A strategy with a real export COMMITS one as `python/<name>/exports/golden/` (2026-09-10)** — step 15 then runs its gate on every clone. The manifest carries the MEASURED warm-up, NAMES the gate when `tools/` has two, and says what the chart's settings did not exercise. A caveat the gate wants seen on a green run prints as a `⚠` line: the runner drops everything else. SOS Fade and the extreme leg have one; B-LEG and BOS do not yet.
