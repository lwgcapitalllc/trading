...
│   ├── b_leg/            (B-LEG bot — the late-retrace setup, split out to run parallel to A+; own CLAUDE.md)
...
└── tradingview/    ← Pine v6 `strategy()` source — TradingView Strategy Tester only
    ...
    ├── sos_fade_strategy.pine + sos_fade_strategy_export.pine        (A+ SOS Fade; the LIVE bot's twin)
