# `extreme_leg_strategy.pine` — the run INTO the shift of structure
...
`h4_sweep_strategy.pine`, renaming the type and method and swapping the four bar globals for
...
✅ Fixed by a per-bar just-entered flag, which is what `h4_sweep_strategy.pine` has always
...
`realign_strategy.pine` has, and every number it produces will be a lab finding until a twin
...
🔴 **THE PARAGRAPH BELOW'S NUMBERS COULD NOT BE REPRODUCED AND SHOULD NOT BE QUOTED. No file in
this repository's history reads the transitioning label for this strategy**, so the 24-trades /
...
`strategies/python/extreme_leg/CLAUDE.md` → *The two cuts TradingView cannot make*.
...
minute it broke: `strategies/python/extreme_leg/CLAUDE.md` → *The two cuts TradingView cannot
make*.
...
The Python port landed (`strategies/python/extreme_leg/`), which meant the Pine's decisions and
...
Each window now names its own city, which is what `indicators/engines/mpc_jarvis.pine` — the
...
off `extreme_leg_strategy_export.pine` — is the one step no machine here can take**, and until
