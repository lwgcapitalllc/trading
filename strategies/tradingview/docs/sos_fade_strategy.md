# sos_fade_strategy.pine — commentary
...
**Covers:** `sos_fade_strategy.pine`, `sos_fade_strategy_export.pine`

---

## [1] SOS FADE STRATEGY — backtest wrapper around the MPC-JARVIS engine

```
// ============================================================================
//  SOS FADE STRATEGY — backtest wrapper around the MPC-JARVIS engine
// ============================================================================
// Same market-structure / fib / liquidity / FVG / RSI-divergence engine as
// mpc_jarvis.pine (kept byte-identical so the SOS Fade sequence stays at parity),
// converted from indicator() to strategy() and given an execution layer at the
// end of the file. It trades ONLY the confirmed SOS Fade reversal sequence:
...
// caps. The Cycle Fib's TRACKING survives and is not optional — it is the SOS Fade
// sequence's HTF POI. All of them still draw in mpc_jarvis.pine.
// ============================================================================
```
...
## [3] Four corners, not nine (Aaron, 2026-08-04) — synced from mpc_jarvis.p

```
// Four corners, not nine (Aaron, 2026-08-04) — synced from mpc_jarvis.pine. The six
...
//  the same reason. They live on in mpc_jarvis.pine if the drawing is ever
//  wanted back.)
//============================================================
...
// FVG minimum-gap floor, SPLIT BY TIMEFRAME — ported from mpc_jarvis.pine
...
// at 15m this file's behaviour must not move — it is the SOS Fade baseline and the
// sos_fade parity reference. ONLY sub-15m changes.
```

## [10] Middle-bar close-cleared test. mpc_jarvis.pine has this OFF at every

```
// Middle-bar close-cleared test. mpc_jarvis.pine has this OFF at every
...
// assistant, on at 15m+ so the SOS Fade baseline and the Python parity pin
// (EngineConfig.fvg_require_close = True) are untouched.
```

## [11] SOS FADE SEQUENCE INPUTS
...
## [12] SOS Fade DEBUG (bar-replay diagnostics — no effect on trades)
...
// (macro_origin / macro_extreme, locked and extended further down) are the SOS Fade
...
## [40] ── EQ APPEARANCE, SYNCED FROM mpc_jarvis.pine (Aaron, 2026-08-04) ──

```
            // ── EQ APPEARANCE, SYNCED FROM mpc_jarvis.pine (Aaron, 2026-08-04) ──
...
```
    ...
    // Measured on 40,000 real M15 bars, that is exactly what it did — the SOS Fade bot lost 2 setups
...
## [47] Live confluence flags for the SOS Fade setup row
...
//  their _loc bars stay: the SOS Fade HTF POI reads them.)
```
...
// HTF POI the SOS Fade sequence reads. The drawing that used to sit alongside it (and
...
//  was never read by the SOS Fade POI or the B-LEG context line, both of which gate on
...
// came from. Recovered from `b25789d~1`, NOT rewritten from memory — this is the SOS Fade
// file's OWN table, not the near-identical fork that survives in
// `b_leg_strategy.pine`, so its rows describe SOS Fade state exactly as they used to.
...
// ── PALETTE REFRESH (Aaron, 2026-08-04) — synced from mpc_jarvis.pine ──
...
// mpc_jarvis.pine). The first column is a spanning tag (SETUP / BIAS / LIQ / STR)
...
// owns the once-only rule. SOS FADE and Weekly can hardcode theirs because they always
...
## [81] SOS FADE SEQUENCE — sweep → SOS → fib entry, IN ORDER
...
## [94] SOS Fade leg resolution: completion (TP3), invalidation (close past 1.0 or fib
...
```
        ...
        // but it was never what the column held on the SOS FADE row, which carries the
        ...
        // EMPTY — the row labels under it (SOS Fade, W, D, BSL, EXT) are named by their group
...
## [106] SOS FADE — the reversal sequence ONLY (sweep → SOS → entry). Once TP3
...
## [111] STRATEGY EXECUTION — SOS Fade sequence entries + scaled fib-target exits
...
## [162] PARITY EXPORT — per-bar DECISION STREAM  (for compare_strategy.py)  _(only in sos_fade_strategy_export.pine)_

```
//============================================================================
//  PARITY EXPORT — per-bar DECISION STREAM  (for compare_strategy.py)
//============================================================================
// This file is `sos_fade_strategy.pine` + THIS appended block, nothing else changed. The
// trade logic above must stay BYTE-IDENTICAL to `sos_fade_strategy.pine` — when your brother
// re-pastes the strategy, regenerate this file (copy the strategy, re-append this block).
//
// REGENERATED 2026-07-30 from `sos_fade_strategy.pine`, to pick up the MINIMUM STOP DISTANCE
...
// it, which was the one KNOWN Pine↔Python divergence on the SOS Fade pair: the parent could refuse
...
//
// Earlier — REGENERATED 2026-07-26 from `sos_fade_strategy.pine` (post-SVP-orphan fix). The previous copy
...
//   sed -n '1,<line before the DIAGNOSTIC LOG header>p' sos_fade_strategy.pine  > new
//   sed -n '<the two blank lines before this PARITY EXPORT header>,$p' this-file >> new
// then (a) restore `strategy("SOS Fade Strategy Export"` on line 29 — the ONLY intended
...
//   python strategies/python/sos_fade/tools/compare_strategy.py <that.csv> --warmup N
```

## [163] DECISION STREAM (packed — compare_strategy.py unpacks with the same sche  _(only in sos_fade_strategy_export.pine)_
...
## [164] BLOCKED TRADES (packed) — the setups that were ready to rest a limit and  _(only in sos_fade_strategy_export.pine)_
...
## [165] CONFIG (packed). cfg_bits packs 16 booleans (4096 = execConfSZ, 2026-07-  _(only in sos_fade_strategy_export.pine)_
...
## [166] EXIT LADDER — the levers that decide where the stop and the partials sit  _(only in sos_fade_strategy_export.pine)_
...
## [167] MINIMUM STOP DISTANCE (2026-07-30) — an ENTRY filter, so it is deliberat  _(only in sos_fade_strategy_export.pine)_
...
## [168] TIME STOP (2026-08-05) — an EXIT lever, but kept out of cfg_exitmode for  _(only in sos_fade_strategy_export.pine)_
...
## [169] NO-FVG ARM GATE (2026-08-10) — `execNoGapArm`. It gates the FALLBACK ent  _(only in sos_fade_strategy_export.pine)_
...
## [170] EQ/FVG COUPLING (2026-08-06) — `eqExemptFvg`. A gap sitting on an active  _(only in sos_fade_strategy_export.pine)_
...
## [171] DIAGNOSTIC (temporary — pins the SOS Fade Stage-1 arming gap vs the Python bot  _(only in sos_fade_strategy_export.pine)_

```
// DIAGNOSTIC (temporary — pins the SOS Fade Stage-1 arming gap vs the Python bot at H4/
...
## [172] DIVERGENCE INTERNALS (temporary — the arm gap above traces to a bear div  _(only in sos_fade_strategy_export.pine)_
...
## [173] FIB LIVE-ANCHOR (temporary — an arm gap traces to the fib entry band; th  _(only in sos_fade_strategy_export.pine)_
