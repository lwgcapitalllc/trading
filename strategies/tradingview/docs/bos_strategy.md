...
**Covers:** `bos_strategy.pine`, `bos_strategy_export.pine`

---

## [1] BOS STRATEGY — the break-of-structure CONTINUATION setup
...
// sos_fade_strategy.pine", and that is already stale: on 2026-07-28 the parent
...
// every caveat: docs/BOS_OPTIMIZATION.md.
//
...
// mpc_jarvis.pine has it OFF at every timeframe; this file had it ON at 15m+.
...
//
// Three things differ from sos_fade_strategy.pine and nothing else:
...
// the panel reads in plain English, like sos_fade_strategy.pine's does.
```
...
// grey out together. That is NOT how sos_fade_strategy.pine treats its equivalents —
...
// $100k start (strategies/python/bos/bos_optimization.md, Run 1).
//
...
// (F9 is STALENESS / bosMaxDays in docs/BOS_SPEC.md §4 — this is F10, not F9.)
// ⚠ IT IS THE SESSION VWAP AND DELIBERATELY THE ONE ON THE CHART. `ta.vwap(hlc3)` is the
// single line mpc_jarvis.pine draws and the one engines/vwap/ is the canonical Python
...
## [102] cat /tmp/blk >> <export>  _(only in bos_strategy_export.pine)_
...
// strategy is being tuned on. `strategies/python/bos/` was DELETED on 2026-08-04
...
## [103] ── STRUCTURE + ARM STATE ───────────────────────────────────────────────  _(only in bos_strategy_export.pine)_
...
## [104] AGE, never the raw bar index — see the warning at the top of this block.  _(only in bos_strategy_export.pine)_
...
## [105] ── THE ANCHOR — four numbers that regenerate every level ───────────────  _(only in bos_strategy_export.pine)_
...
## [106] ── THE GATES — why a ready setup was refused ───────────────────────────  _(only in bos_strategy_export.pine)_
...
## [107] ⚠ THE VOLUME IS PLOTTED, NOT ASSUMED, and this file shipped without it b  _(only in bos_strategy_export.pine)_
...
## [108] ── THE TRADE ───────────────────────────────────────────────────────────  _(only in bos_strategy_export.pine)_
...
## [109] ── CONFIG — the run describes ITSELF, which is what stops a green gate l  _(only in bos_strategy_export.pine)_
...
## [110] Enums packed as DECIMAL DIGITS, each field < 10, so decoding is division  _(only in bos_strategy_export.pine)_
