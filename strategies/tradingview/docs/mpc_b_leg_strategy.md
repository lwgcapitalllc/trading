# mpc_b_leg_strategy.pine — commentary
...
**Covers:** `mpc_b_leg_strategy.pine`, `mpc_b_leg_strategy_export.pine`

---

## [1] MPC B-LEG STRATEGY — the late-retrace setup, split out to run PARALLEL t

```
// ============================================================================
//  MPC B-LEG STRATEGY — the late-retrace setup, split out to run PARALLEL to A+
// ============================================================================
// FORKED FROM mpc_strategy.pine (the "MPC A+ Strategy"). Same byte-identical
...
// mpc_assistant.pine / mpc_strategy.pine.
// ----------------------------------------------------------------------------
...
// Kept in LOCKSTEP with mpc_strategy.pine's block of the same name — same eight
...
// behaviour, it just no longer owns the declarations. Mirrors mpc_strategy.pine.
//
...
// than the Structure trail — only equal or tighter. Copied verbatim from mpc_strategy.pine
...
// climbing off it. na = not engaged yet. (Ported from mpc_strategy.pine: the structure
...
// Ported from `mpc_strategy.pine` in the same commit; both forks share ONE exit ladder.
```
...
## [133] PARITY EXPORT — the B-LEG decision stream as plotted columns  _(only in mpc_b_leg_strategy_export.pine)_

```
//============================================================
//  PARITY EXPORT — the B-LEG decision stream as plotted columns
//============================================================
// THIS FILE = mpc_b_leg_strategy.pine + THIS appended block, nothing else changed
// except the strategy() title on line 40. The instrumented twin of the B-LEG
// strategy, the same way mpc_strategy_export.pine is the A+ strategy's twin.
//
// Export it to CSV from a 15m XAUUSD chart ("Export chart data"), then:
//   command-center/backend/.venv/bin/python \
//     strategies/python/mpc_bleg/tools/compare_bleg.py <that.csv> --warmup N
...
## [134] DECISION STREAM (packed — compare_bleg.py unpacks with the same scheme):  _(only in mpc_b_leg_strategy_export.pine)_
...
## [135] The frozen ladder of the OPEN trade. The B LEG computes its own TP1/TP2   _(only in mpc_b_leg_strategy_export.pine)_
...
## [136] B-LEG TRACKER STATE — the whole point of this harness (see the note abov  _(only in mpc_b_leg_strategy_export.pine)_
...
## [137] CONFIG (packed) — the SAME scheme mpc_strategy_export.pine uses, so  _(only in mpc_b_leg_strategy_export.pine)_

```
// CONFIG (packed) — the SAME scheme mpc_strategy_export.pine uses, so
...
## [138] TIME STOP (2026-08-05) — the shared exit ladder's clock lever, carried h  _(only in mpc_b_leg_strategy_export.pine)_
...
## [139] EQ/FVG COUPLING (2026-08-06) — `eqExemptFvg`, a gap on an active EQH/EQL  _(only in mpc_b_leg_strategy_export.pine)_
...
// ⚠ It defaults **false** in this fork and **true** in `mpc_strategy.pine`, and that fork is
// real, not drift — which is exactly why the column has to exist HERE too. `mpc_bleg` pins the
