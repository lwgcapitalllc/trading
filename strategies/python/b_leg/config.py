...
# strategies/python on path so `sos_fade` imports by bare name (same shim the tests use).
...
class BLegConfig(SosFadeConfig):
    ...
    #   `_place_entries`, which `BLegExecution` overrides, and `b_leg_strategy.pine` has no such
    ...
    #   which `BLegExecution` overrides, and `b_leg_strategy.pine` has no such input to be
