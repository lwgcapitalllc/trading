...
from b_leg import BLegConfig, BLegTracker, BLegStrategy  # noqa: E402
...
def test_driver_runs_end_to_end():
    strat = BLegStrategy().run(synth_bars(12), warmup=100)
...
def test_driver_is_deterministic():
    a = BLegStrategy().run(synth_bars(8))
    b = BLegStrategy().run(synth_bars(8))
...
def test_longs_and_shorts_off_means_no_trades():
    strat = BLegStrategy(BLegConfig(exec_longs=False, exec_shorts=False)).run(synth_bars(12))
...
def test_this_fork_records_no_missed_setups():
    ...
    strat = BLegStrategy().run(synth_bars(30), warmup=100)
...
def test_this_fork_redefaults_the_time_stop_and_the_parent_keeps_its_own():
    ...
    """
    from sos_fade import SosFadeConfig
...
def test_the_time_stop_only_ever_fires_before_tp1():
    ...
    for name in ("b_leg_strategy.pine", "b_leg_strategy_export.pine"):
        src = (_ROOT / "strategies" / "tradingview" / name).read_text()
...
def test_the_meta_descs_are_the_pine_tooltips_verbatim():
    ...
    pine = (_ROOT / "strategies" / "tradingview" / "b_leg_strategy.pine").read_text().splitlines()
    meta = json.loads((_ROOT / "strategies" / "python" / "b_leg"
                       / "b_leg.meta.json").read_text())
