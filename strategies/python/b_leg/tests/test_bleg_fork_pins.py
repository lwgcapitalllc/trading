...
from b_leg import BLegStrategy  # noqa: E402
...
def test_the_fork_pins_the_eq_fvg_coupling_OFF_where_the_parent_pins_it_ON():
    ...
    in the commit that ports the input into `b_leg_strategy.pine`, then re-run `compare_bleg.py`.
    """
    assert SosFadeStrategy.engine_config().eq_exempt_fvg is True
    assert BLegStrategy.engine_config().eq_exempt_fvg is False
...
def test_the_fork_inherits_every_OTHER_engine_pin_from_the_parent():
    ...
    parent = SosFadeStrategy.engine_config()
    fork = BLegStrategy.engine_config()
