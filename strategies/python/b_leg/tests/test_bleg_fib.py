...
from b_leg.execution import _FIB_RATIOS, _band_fib  # noqa: E402
from sos_fade.execution import _FIB_RATIOS as _APLUS_RATIOS  # noqa: E402
...
def test_the_tracker_freezes_the_leg_with_the_band_and_re_freezes_on_migration():
    ...
    from b_leg import BLegConfig, BLegTracker

    tr = BLegTracker(BLegConfig(), tf_seconds=900)

    def sig(index, close, high, low, **kw):
        base = dict(index=index, close=close, high=high, low=low, bull_sos=False, bear_sos=False,
                    bull_bos_high=None, bull_bos_low=None, bear_bos_high=None, bear_bos_low=None,
                    bull_bos_high_ms=None, bull_bos_low_ms=None,
                    bear_bos_high_ms=None, bear_bos_low_ms=None)
        base.update(kw)
        return SimpleNamespace(**base)

    seq = SimpleNamespace(bleg_arm_l=False, bleg_arm_s=False)

    st = tr.update(sig(0, 1050, 1100, 1000, bull_sos=True, bull_bos_high=1100.0,
                       bull_bos_low=1000.0, bull_bos_high_ms=2000, bull_bos_low_ms=1000), seq)
    ...
    st = tr.update(sig(1, 2050, 2100, 2000, bull_sos=True, bull_bos_high=2100.0,
                       bull_bos_low=2000.0, bull_bos_high_ms=9000, bull_bos_low_ms=8000), seq)
...
def test_the_recorded_leg_is_not_the_tracking_target():
    ...
    seq = SimpleNamespace(bleg_arm_l=False, bleg_arm_s=False)
    tr.update(sig(0, 1050, 1100, 1000, bull_sos=True, bull_bos_high=1100.0, bull_bos_low=1000.0,
                  bull_bos_high_ms=2000, bull_bos_low_ms=1000), seq)
