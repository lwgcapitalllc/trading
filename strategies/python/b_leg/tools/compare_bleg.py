...
The B-LEG twin of `sos_fade/tools/compare_strategy.py`. Reads a TradingView
"Export chart data" CSV of `strategies/tradingview/b_leg_strategy_export.pine` — the instrumented
...
def config_from_export(df: pd.DataFrame, base: Optional[BLegConfig] = None) -> BLegConfig:
    ...
    if "cfg_bleg_days" in df.columns and not pd.isna(row["cfg_bleg_days"]):
        ...
        print("WARNING: no cfg_bleg_days column — the B-LEG staleness cap is assumed to be at "
              "its default. Re-export off the current b_leg_strategy_export.pine.")
    return cfg


def _expand(df: pd.DataFrame) -> pd.DataFrame:
    """Unpack the export's packed columns into the flat names the diff loop reads.
    Mirrors b_leg_strategy_export.pine's PARITY EXPORT scheme exactly."""
    df = df.copy()
...
def run_parity(path, warmup: int = 0, price_tol: float = 0.01, r_tol: float = 0.02,
    ...
    strat = BLegStrategy(cfg).run(bars, engine_config=eng, warmup=0)
...
def main() -> int:
    ...
    # mismatch — see the long note in `sos_fade/tools/compare_strategy.py`.
    ...
    if _tf is not None:
        print(f"CANNOT DIFF - {_tf.replace('sos_fade_strategy_export.pine', 'b_leg_strategy_export.pine')}")
        return 2
