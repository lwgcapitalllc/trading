...
2026-08-26, NEVER MEASURED ON — see strategies/python/mpc_sos_fade/docs/STASHED_EXIT_LADDER_EXPERIMENTS.md`
...
- **28 new tests across three files, all green**, and the whole A+ suite was 555 green with the
  work applied (measured 2026-08-27, `python3 -m pytest strategies/python/mpc_sos_fade/tests/ -q`).
...
1. **A paragraph in `strategies/python/mpc_sos_fade/CLAUDE.md`.** There is nothing there about
...
- **The live bot is untouched, and pulling this stash cannot change that.** The frozen deployment
  at `algos/markets/fx/instances/mpc_sos_fade_demo/deployed/` contains none of it, and only a
...
```
M  strategies/python/mpc_sos_fade/config.py
M  strategies/python/mpc_sos_fade/execution.py
M  strategies/python/mpc_sos_fade/mpc_sos_fade.meta.json
?? strategies/python/mpc_sos_fade/tests/test_vol_rung.py
?? strategies/python/mpc_sos_fade/tests/test_sec_tp2_pct.py
?? strategies/python/mpc_sos_fade/tests/test_trail_at_tp1.py
```
