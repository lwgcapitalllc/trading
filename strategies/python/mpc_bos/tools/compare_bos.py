...
`indicators/mpc_bos_strategy_export.pine` — the instrumented strategy that plots its per-bar
...
def _require(df: pd.DataFrame, columns) -> None:
    ...
    if missing:
        raise ExportIncomplete(
            ...
            "indicators/mpc_bos_strategy_export.pine — a column added to the Pine after an "
