...
Ports `strategies/tradingview/mpc_bos_strategy_export.pine` lines 3446-3629 plus the session-VWAP gate
...
is the same one `mpc_bleg` uses, and it exists for the same reason: the tracker is where every
...
def _atr_of(sig) -> Optional[float]:
    ...
    `MpcBosStrategy` stamps it onto the state before the tracker runs.
    """
