...
The fourth of the family, beside `sos_fade/tools/compare_strategy.py`,
`b_leg/tools/compare_bleg.py` and `bos/tools/compare_bos.py`. It reads a TradingView
"Export chart data" CSV of `strategies/tradingview/extreme_leg_strategy_export.pine`, replays
...
🔴 **UNTIL THIS EXITS 0 ON A REAL EXPORT, NOTHING IN `strategies/python/extreme_leg/` IS
...
can agree on the shape of an edge and disagree about a trade. `realign` in this repo has no
...
differences of rule rather than bugs on either side:
  ...
  1. **The previous WEEK's level.** The Pine's own sweep tracking treats every family the same way
     ...
     port consumes and which is 100% Pine-parity-validated against `mpc_jarvis.pine`, wants a
...
def _wrong_export(path: Path) -> "str | None":
    ...
    if "trade number" in cols:
        ...
        what = "not an export of extreme_leg_strategy_export.pine"
    ...
    return (
        ...
        f"    1. Paste strategies/tradingview/extreme_leg_strategy_export.pine onto XAUUSD 5m\n"
