...
from extreme_leg.execution import (  # noqa: E402
    BLK_ATR_NOT_READY,
...
from extreme_leg.htf import HtfStructure  # noqa: E402
from extreme_leg.strategy import LegState, _pine_round  # noqa: E402
...
def _ladder(**over):
    ...
    return ExtremeLegStrategy._ladder(cfg, args["is_friday"], args["target"], args["entry"],
...
def test_two_families_must_agree_on_the_SAME_bar():
    ...
    """
    s = ExtremeLegStrategy(ExtremeLegConfig(min_families=2))
...
def test_a_family_that_is_switched_off_does_not_arm_anything():
    ...
    s = ExtremeLegStrategy(ExtremeLegConfig(use_weekly_level=False))
...
def test_an_arming_expires_and_the_two_sides_are_independent():
    ...
    s = ExtremeLegStrategy(ExtremeLegConfig())
...
def test_the_average_range_is_unknown_until_it_has_a_full_window():
    ...
    s = ExtremeLegStrategy(ExtremeLegConfig(), atr_length=5)
    for i in range(4):
...
def test_the_pines_hardcoded_constants_are_not_lab_settings():
    ...
    assert ExtremeLegStrategy(atr_length=7).atr_length == 7
...
def test_the_declaration_matches_what_the_constructor_actually_does():
    ...
    """
    from strategies.python.extreme_leg import LAB_STRATEGY
...
def test_a_cut_that_is_OFF_cannot_refuse_even_when_its_answer_is_yes():
    ...
    code = ExtremeLegStrategy._ladder(
...
def test_each_cut_refuses_with_its_OWN_code_when_switched_on():
    ...
    assert ExtremeLegStrategy._ladder(
