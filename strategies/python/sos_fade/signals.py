...
1. **recentSSL / recentBSL** (sos_fade_strategy.pine 3324-3373 + 3606-3636) — the "most
...
class SignalAdapter:
    ...
    def _update_liquidity(self, index: int, t: int, liq_events) -> None:
        """Reproduce sos_fade_strategy.pine 3343-3374: latch each source's swept slot on
    ...
    def _resolve_recent(self, slots) -> Tuple[str, Optional[int], Optional[int]]:
        """sos_fade_strategy.pine 3606-3636 — pick the pool swept on the LATEST bar, in
    ...
    **This is THE seam, and it exists so there is exactly one of it.** Both consumers of a zone —
    ...
    if want_ob and not sig.obs_available:
        raise PoiSourceUnavailable(
            ...
            f"EngineConfig(order_blocks=True) — SosFadeStrategy.run() does this from the config, "
...
def sos_aware_veto(sig: Signals, l_sos_bar, s_sos_bar) -> Tuple[bool, bool]:
    """Pine `longVetoA` / `shortVetoA` (sos_fade_strategy.pine ~3701, changed 2026-07-21).
