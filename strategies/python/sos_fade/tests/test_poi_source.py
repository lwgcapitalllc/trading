...
from strategies.python.sos_fade.signals import (POI_RANK_FVG, POI_RANK_FVG_ON_OB,
...
def test_the_confirming_block_must_point_the_SAME_WAY_as_the_gap():
    ...
    and it must be flipped here and in `sos_fade_strategy.pine` together or the parity gate goes red."""
...
def test_the_stack_builds_the_order_block_engine_ONLY_when_the_config_needs_it():
    ...
    for source in ("Order block", "Either", "FVG first"):
        strat = SosFadeStrategy(config=_cfg(source))
...
def test_stack_config_preserves_every_other_engine_pin():
    ...
    got = SosFadeStrategy(config=_cfg("Order block")).stack_config()
...
def test_the_new_mode_is_in_the_lab_choices_and_the_wire_format():
    ...
    import json
    import pathlib
    from strategies.python.sos_fade.tools.compare_strategy import _POI_SOURCE
