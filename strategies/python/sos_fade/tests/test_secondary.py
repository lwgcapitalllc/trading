...
from strategies.python.sos_fade.secondary import Structure1m
...
from strategies.python.sos_fade.secondary import M1State, SecArm, SecondaryArm
...
def test_run_dual_builds_a_1m_sig_carrying_every_field_advance_stage_reads():
    ...
    try:
        SosFadeStrategy(config=SosFadeConfig(exec_min_atr_pct=0.0, exec_secondary=True, symbol="XAUUSD"),
...
def _fill_secondary(entry, sl, tp1, tp2, direction=1, **cfg_kw):
    ...
    from strategies.python.sos_fade.execution import Decision, _Pending
...
def _primary_qty(monkeypatch, **cfg_kw):
    ...
    """
    from strategies.python.sos_fade import execution as _ex_mod
    cfg = SosFadeConfig(exec_min_atr_pct=0.0, exec_secondary=True, **cfg_kw)
    ex = Execution(cfg, initial_capital=100_000.0)
...
# configuration
# (`disable_if` in `sos_fade.meta.json`), and the lab stops perturbing it in sensitivity for
