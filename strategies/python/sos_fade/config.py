...
class SosFadeConfig:
    ...
    #   PYTHON-ONLY, NO PINE COUNTERPART (2026-08-10). `sos_fade_strategy.pine` has `execReqFVG` and
    ...
    #   rather than left to be read as current. `sos_fade_strategy.pine` and its export mirror carry
    ...
    #   version is wanted — and it must be flipped in `sos_fade_strategy.pine` in the same commit.
    ...
    #   ⚠ NO PINE COUNTERPART. `sos_fade_strategy.pine`'s `execSlLevel` is an `input.string` with five
    ...
    #   ✅ **PORTED to `sos_fade_strategy.pine` the same day** as `execMinAtrPct` + `f_marketHasRange`,
    ...
    #   `strategy.exit` with qty_percent = 0 closes the WHOLE position) — see sos_fade_strategy.pine.
    #   Measured: `sos_fade_optimization.md` Run 1 — 0/0 = 70.7R vs 47.9R at 30/40, monotonic
    ...
    #   Full rules: docs/SOS_FADE_SECONDARY.md. There is NO Pine parity gate for it — the Pine
    ...
    #   by `exec_sec_tp_r`, which exists nowhere in `sos_fade_strategy.pine` — the Pine has no re-entry
    ...
    #   the feature IS, not just how often it fires. See `sos_fade_optimization.md` → the
    ...
    #   MEASURED over sos_fade's 62 real losses, XAUUSD M15 2018-09-14 → 2026-08-14, both
