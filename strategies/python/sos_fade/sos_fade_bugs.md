...
sweeps and "which setting is better" questions belong in `sos_fade_optimization.md`.

A bug is only closed when all four are true: the fix is in `execution.py` (or wherever it lives),
the same fix is in `strategies/tradingview/sos_fade_strategy.pine` AND `strategies/tradingview/sos_fade_strategy_export.pine`,
...
| where | line |
|---|---|
| `strategies/python/sos_fade/execution.py` | `_advance_stage`, ~1312 (`if self._stage < 1 and sig.high >= self._tp1`) |
| `strategies/tradingview/sos_fade_strategy.pine` | 4546 (`if lStage < 1 and high >= lTP1`) |
...
3. **Port to `sos_fade_strategy.pine` AND `sos_fade_strategy_export.pine` in the same commit**, then re-run
...
Measurement record: `sos_fade_optimization.md` → Run 13.
