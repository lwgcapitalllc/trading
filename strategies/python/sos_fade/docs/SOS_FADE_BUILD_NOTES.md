...
use, not what it does, and "A+" would collide the moment a second bot also traded A+ setups.
...
`strategies/python/b_leg/` (the standalone B-LEG bot) reuses this package's engine + A+ sequence +
...
- **Do** port any change to `sos_fade_strategy.pine`'s A+ block or execution layer here line-for-line, then
...
- Pine source of truth: `strategies/tradingview/sos_fade_strategy.pine` (A+ block ~3708-3972, execution ~4112-4735).
