# SOS Fade Build Notes

**Status:** ARCHIVE - relocated history, deliberately. Implementation-level history and
war-story detail moved OUT of `strategies/python/sos_fade/CLAUDE.md` on 2026-08-12 so that file could stay standing rules
...
**Why it moved.** `strategies/python/sos_fade/CLAUDE.md` had grown to 207,701 bytes, most of it dated narrative,
...
`Trade.mfe_price` on all 158 SOS Fade trades to 0.0000R, and two mutations were watched red (crediting
...
against the SOS Fade book's 1.40R; 84% stopped out against 61%. **At 5R the two pools are identical
...
|---|---|---|---|---|---|
| SOS Fade shipped | 158 | +130.8 | +0.828 | −6.0R | 7.0h |
...
+23.3R of its +22.5R lands in the 32 months SOS Fade was down. It still does not help:

| | end | worst drawdown | days under water |
|---|---|---|---|
| SOS Fade alone | 2590x | −50% | 1813 |
...
pool on the SOS Fade exits, because capping at 2R discards the tail that was carrying it. Sweeping the
...
`sos_fade_optimization.md` for the full tables, run ids and basis.** Headline: best variant
...
Rules live in `strategies/python/sos_fade/CLAUDE.md` → *The DEAD-MARKET floor*. This is what
...
because two entry paths call it. `bos` overrides `_place_entries`, which read as *it cannot
reach this* — and it calls the shared floor check from inside its own placer
(`bos/execution.py:401`). Its own suite passed while silently acquiring a volatility filter
...
**The guard is now PORTED (2026-07-30) — it was the one known Pine↔Python divergence on the SOS Fade
...
`sos_fade_strategy_export.pine` that `compare_strategy.py` decodes. See `### The minimum-stop guard`.
...
1. No validation that the chosen SL fib is on the correct side of the entry, or a sane distance
   from it. Assume `sos_fade_strategy.pine` has the same exposure (same dropdown) until checked.
...
**Every sweep of these levers is logged in `sos_fade_optimization.md`** — one entry per run,
...
`grep -c '^# Run ' sos_fade_optimization.md`, never off this line** — it read "twelve" while
...
1. **TP split** (21 combos) — monotonic; best is `exec_tp1_pct=0, exec_tp2_pct=0` (100% on the
   ...
   lockstep across `config.py` and both SOS Fade Pine files. The tick-mode re-run this entry originally
...
`sos_fade_strategy.pine` gained a structure-based runner trail, a TP2 stop-floor dropdown, an SL fib
...
- **`exec_fvg_50` is NOT ported** (same standing as `exec_conf_sz`) — `compare_strategy.py` refuses
  an export taken with it on. `exec_bleg` on is refused too: those trades belong to `b_leg`.
...
| `sos_fade_strategy_export.pine` | REGENERATED off the parent (body now byte-identical again apart from line 29's title) + `cfg_min_stop` / `cfg_min_stop_val` plots |
| `compare_strategy.py` | `_MIN_STOP` decode; **absent column ⇒ `"Off"`**, never the Python default |
| `sos_fade.meta.json` | both fields, with `show_if` on the mode (which needed `show_if` to accept a LIST of values — one enum, three ON states) |
| `b_leg/config.py` | `exec_min_stop_mode` PINNED `"Off"` — that fork overrides `_place_entries`, so the floor never runs there and its Pine has no such input |
...
trusting a run made with the guard on: re-paste `sos_fade_strategy_export.pine`, export at the mode you
...
**Root cause — an unpinned engine input.** `sos_fade_strategy.pine` HARDCODES the middle-bar close-cleared
...
`mpc_jarvis.pine`, where it IS an input and IS off). Nothing exported it, so nothing caught it — the
...
False, so no other consumer moves) and `SosFadeStrategy.engine_config()` pins it **True**, alongside
...
window is drift, not a bug. `sos_fade_strategy_export.pine` now carries `cfg_bits` bits 16384 /
...
`sos_fade_strategy_export.pine` was last regenerated 2026-07-15 and had drifted on three trade-affecting
Pine changes, so any diff it produced was July-15 drift, not a bug. Regenerated from
`sos_fade_strategy.pine` @ `361f007`:
...
**Slippage pinned to 0 in the Pine (2026-07-23).** Both `sos_fade_strategy.pine` and `sos_fade_strategy_export.pine`
...
**When the Pine changes:** brother re-pastes `sos_fade_strategy.pine` → regenerate `sos_fade_strategy_export.pine`
...
(**ANSWERED 2026-07-26, Run 3 in `sos_fade_optimization.md`: it does not — it pays for itself**); and
...
`strategies/tradingview/sos_fade_strategy.pine`, minus Pine's leading `   ↳ ` indent marker. Every `desc` is that
...
use, not what it does, and "SOS Fade" would collide the moment a second bot also traded SOS Fade setups.

**"SOS Fade" is still correct vocabulary and is deliberately kept** wherever it names the brother's own
Pine concept — the SOS Fade/B/C/D grade dropdown, the "SOS FADE SEQUENCE" block this ports, and the
...
A counter-trend reversal that fades exhaustion at HTF liquidity. Three-stage SOS Fade sequence: **Arm**
...
TP1, stop→TP1 on TP2, and a ratcheting trail on the runner. Full rules: `docs/SOS_FADE_SPEC.md`.
...
(backtest.replay)             (Pine-named inputs)          (SOS Fade stages)               (orders + fills + R)
```
...
- **`signals.py`** — `SignalAdapter`: turns a replay `BarState` into the exact Pine-named globals the
  SOS Fade block reads. Two reconstructions are non-trivial and must stay faithful:
...
- **`execution.py`** — `Execution`: entry edge → resting limit → TP1/TP2/runner ladder → staged stop
  ...
  Pine's pink `TRADE BLOCKED` tag (`sos_fade_strategy.pine` 4025-4086): a setup price and the engine had
  ...
  **Reporting-only and parity-safe**, exactly like the excursion fields:
  ...
  computed, never recomputing them), which is why `b_leg` gets none — it overrides that method,
  and those codes describe why an *SOS Fade* setup was refused. Surfaced on the lab price chart's Blocked
  ...
  `sos_fade_strategy.pine` 3064-3194 + 4022-4023). See *The missed-setup watch* below.
- **`strategy.py`** — `SosFadeStrategy`: the driver. `run(df, warmup=…)` replays a canonical frame
...
flat) and why `b_leg` needs the explicit `_records_misses = False` opt-out rather than getting
...
**Full rules + design: `docs/SOS_FADE_SECONDARY.md`.** One paragraph: after the **primary** 15m
SOS Fade trade on a leg has traded and gone flat, while the 15m div + SOS are still live and price is back
...
three groups in `sos_fade.meta.json`, and the rows that cannot matter are **greyed with the
...
The register of how this bot (and `b_leg`, which reuses the whole ladder) decides where the
stop and the targets sit. Keep it current: a new exit lever in the Pine lands here, in `config.py`,
in `sos_fade_strategy_export.pine`, and in `compare_strategy.py` in ONE commit.
...
**`b_leg` overrides TP1, TP2 and the SL** with its own band prices (SL = band origin, TP1 =
...
stop distance, regime filtering): `sos_fade_optimization.md` → Run 19.
...
Full grid, the ladder-shape test and the void banner: `sos_fade_optimization.md` → Run 21.
...
on the self-cancelling SWAP rule the Pine fixed on 2026-08-03); `sos_fade` pins
`eq_exempt_fvg=True` and `b_leg` pins it False (that fork's Pine keeps it off); and both export
...
`SosFadeStrategy(..., cost_profile=<AccountProfile>)` now passes a profile straight through to
...
untouched by construction. `b_leg` inherits the kwarg.
...
`granted`. SOS Fade shared then reads **+127.11R against +127.11R solo** — exact.
...
1. **`fvg_max_count=7`** — `sos_fade_strategy.pine` sets Max Active FVGs to 7 (the FVG engine default is 8);
...
2. **`fvg_threshold_pct=0.1`** *(added 2026-07-31 — it had NEVER been pinned)*. The minimum-gap floor.
   `sos_fade_strategy.pine` splits it by timeframe (`fvgThreshLTF` 0.0 below 15m / `fvgThreshHTF` **0.1** at
   15m and up, lines 116-118) and this bot trades 15m. `mpc_jarvis.pine` uses **0.04** at 15m and
...
3. **`fvg_require_close=True`** — `sos_fade_strategy.pine` HARDCODES the middle-bar close-cleared check
...
The standing regression harness (same pattern as the engines' `compare_*.py`). `sos_fade_strategy_export.pine`
(in `indicators/`) = `sos_fade_strategy.pine` + an appended block that plots the per-bar decision stream
...
shipped rule is unchanged and no number in this file moves. Full grid: `sos_fade_optimization.md`
→ Run 24.
...
`sos_fade_strategy.pine` runs a minimum-gap floor of 0.0 below 15m and 0.1 at 15m and above, and
...
```
command-center/backend/.venv/bin/python -m pytest strategies/python/sos_fade/tests/ -q
```
...
`strategies/python/b_leg/` (the standalone B-LEG bot) reuses this package's engine + SOS Fade sequence +
...
1. **`signals.py`** — `Signals` gained `bull_bos_high/low` + `bear_bos_high/low` (the break-leg
   endpoints the B-LEG band-freeze reads). Nothing in the SOS Fade path reads them.
...
3. **`execution.py`** — the SOS Fade arm decision was extracted from `_place_entries` into `_armed()` (a pure
   refactor) so the B-LEG subclass can reuse the "SOS Fade has priority" gate. No behaviour change.

Full context in `strategies/python/b_leg/CLAUDE.md`.
...
|---|---|---|---|---|---|
| SOS Fade shipped | 158 | +130.8 | +0.828 | −6.0R | 32 |
| the pool, SOS Fade exits | 109 | +22.5 | +0.207 | −13.7R | 33 |
...
- **Do** port any change to `sos_fade_strategy.pine`'s SOS Fade block or execution layer here line-for-line, then
...
- **Never** revert the three B-LEG parity-safe additions above without also updating `b_leg/`.



## References

- Spec: `docs/SOS_FADE_SPEC.md`; build plan + order: `docs/SOS_FADE_BUILD_PLAN.md`.
- Pine source of truth: `strategies/tradingview/sos_fade_strategy.pine` (SOS Fade block ~3708-3972, execution ~4112-4735).
...
different questions of the primary: the **gap** half needs the SOS Fade to have scratched at breakeven,
...
|---|---|---|---|---|
| SOS Fade alone | 159 | +139.71R | −5.61R | −45.6% |
| SOS Fade + reclaim (after a stop) | 205 | **+169.71R** | −6.13R | −43.9% |
| SOS Fade + both | 249 | +177.89R | −6.41R | −43.3% |
...
`sos_fade_optimization.md` → the 2026-08-24 run.**
...
The `desc` on `exec_recovery` in `sos_fade.meta.json` opened *"THIS SWITCH DOES NOT MODEL ONE
...
MEASURED end to end on XAUUSD M15 2018-09-14 → 2026-08-14 at `puprime_ecn`: 181 SOS Fade trades
unchanged, 65 recovery trades added, median recovery risk $2,050 against SOS Fade's $10,127. **All 65
