...
- **A result here is a measurement, not a default.** Adopting one means a single commit
  across `config.py`, `strategies/tradingview/sos_fade_strategy.pine`, `strategies/tradingview/sos_fade_strategy_export.pine`
...
`sos_fade_strategy_export.pine` still emits **no `cfg_min_stop*` column**, so parity is proven only at the
...
2. Any default change lands in **one commit** across `config.py`,
   `strategies/tradingview/sos_fade_strategy.pine`, `strategies/tradingview/sos_fade_strategy_export.pine` and
...
**both are hardcoded in Python AND in `sos_fade_strategy.pine`** — this would be new inputs in both,
...
1. **`exec_sl_level` is not a safe dropdown.** Only `"1.0"` (the leg origin) is structurally
   ...
   until a guard exists.** (Unverified here: whether `sos_fade_strategy.pine` has the same exposure. It
...
- **Real:** give the bot a stop whose distance does not come from fib geometry — ATR-based — with
  ...
  inputs in `sos_fade_strategy.pine`.
...
`config.py` AND in `sos_fade_strategy.pine` / `sos_fade_strategy_export.pine` / `compare_strategy.py`, in one
...
`exec_min_stop_pct` (default 0.1) in `config.py` **and** in `strategies/tradingview/sos_fade_strategy.pine` **and**
`strategies/tradingview/sos_fade_strategy_export.pine` (new `cfg_*` column) **and** `compare_strategy.py`
(`_TOGGLE_COLS`), in ONE commit, with `compare_strategy.py` re-run green on a fresh export. Note
`b_leg` inherits `_place_entries` — decide explicitly whether the B-LEG's band-origin stop wants
...
`config.py` **and** `sos_fade_strategy.pine` (which already has `execMinStopMode`/`execMinStopVal`)
**and** a new `cfg_min_stop*` column in `sos_fade_strategy_export.pine` **and** a `_TOGGLE_COLS` entry in
...
`b_leg` reuses), with both parity gates re-run green on full-history post-fix exports.
...
|---|---|---|
| **MAGNITUDE** | how far past the broken level the breaking candle CLOSED | Yes — `bosMinDispAtr` in `bos_strategy.pine` ("Break must clear the swing by x ATR", default 0.5). **Never measured** — the BOS file's own tooltip says so. |
...
2. `SosFadeStrategy.engine_config()` PINS `fvg_require_close = True` specifically to match this
...
- flip `fvgThreshHTF` 0.1 → 0.04 and `fvgReqCloseHTF` ON → OFF in `sos_fade_strategy.pine` **and**
  `sos_fade_strategy_export.pine` in the same commit (the export must never drift from the parent),
...
fit. Confirming it costs one chart: put `mpc_jarvis.pine` and `sos_fade_strategy.pine` on the same
...
0.618 / 0.702 / 0.786. One branch, both directions, in `sos_fade_strategy.pine`, `sos_fade_strategy_export.pine`,
`bos_strategy.pine` (same helper, same hole) and `execution._deep_fib_edge()`.
...
| where | what |
|---|---|
| `sos_fade_strategy.pine` | the entry-edge loop (`longEdge`/`shortEdge`) + the `aplus*_fvg` confluence flag |
...
The toggle was written into `sos_fade_strategy.pine` on 2026-08-02: `execFvgPreZone` (default OFF), a
...
Nothing was ported to Python, so `sos_fade` still takes all 18 trades. Parity is unaffected —
...
- **`b_leg`** is structurally unaffected — its trigger is the band tap and it never reads an FVG.
...
This is the whole record of what was tried, so nobody re-runs it. `docs/OB_FADE_SPEC.md` —
which described a separate `ob_fade` fork — was **DELETED** in the same commit, because the
...
the 2026-08-02 revert that `sos_fade_strategy.pine` records was of a WIDER version that also caught 0.702.
...
- **`exec_sl_deep` at any other boundary.** 0.786 is the shipped threshold and it has already been
  wrong twice in the other direction (see `sos_fade_strategy.pine` ~line 509). This run holds it fixed.
...
2.0 → **0.5**, in lockstep across `config.py`, `sos_fade_strategy.pine` and `sos_fade_strategy_export.pine`.
...
`exec_sec_*` twin in `sos_fade_strategy.pine` beyond the shipped ones, and `algos/live/bridge.py` refuses
...
`sos_fade` at shipped defaults with `exec_secondary=False`, warmup 1000, bar fills, **both
...
$38 median stop** — inert, the `b_leg` trap — and at 0.05% it binds constantly and hands back
...
figure above is a LAB finding. `strategies/tradingview/recovery_strategy.pine` is a FORK for
...
Five new settings on `sos_fade`, all inert unless `exec_be_buf_mode` is moved off `"Ticks"`:
