...
`strategies/python/bos/CLAUDE.md`. **Tuning a parameter whose branch no gate reaches moves
...
- 🔴 **A cut is applied to the SETUP POOL, before any position-slot rule** — refusing a setup
  ...
  `strategies/python/extreme_leg/extreme_leg_optimization.md` → Run 11, where a cut that
...
- **A result here is a measurement, not a default.** Adopting one means a commit across
  `config.py`, `strategies/tradingview/bos_strategy.pine` and its export, with the parity
