...
Three things differ from `sos_fade_strategy.pine` and nothing else (the Pine's own header says so):
...
from sos_fade.execution import BlockedSetup, Execution, _Pending  # noqa: E402
...
class BosExecution(Execution):
    ...
    #   per leg, which is the raw material), not a port. Same call `b_leg` makes.
