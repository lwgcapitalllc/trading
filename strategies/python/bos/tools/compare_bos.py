...
The third of the family, beside `sos_fade/tools/compare_strategy.py` and
`b_leg/tools/compare_bleg.py`. Reads a TradingView "Export chart data" CSV of
`strategies/tradingview/bos_strategy_export.pine` — the instrumented strategy that plots its per-bar
...
🔴 **UNTIL THIS EXITS 0 ON A REAL EXPORT, NOTHING IN `strategies/python/bos/` IS
...
The fix is that `bos_strategy_export.pine` now plots `px_volume` itself, which is the same
...
from bos import BosConfig, BosStrategy  # noqa: E402
from sos_fade.tools.compare_strategy import load_export  # noqa: E402

# ── the packed decoders — MUST match bos_strategy_export.pine's plot scheme ──
...
def _require(df: pd.DataFrame, columns) -> None:
    ...
    if missing:
        raise ExportIncomplete(
            ...
            "strategies/tradingview/bos_strategy_export.pine — a column added to the Pine after an "
...
def compare(csv_path: Path, warmup: int, price_tol: float, r_tol: float) -> int:
    ...
    if cfg.bos_vwap_req != "Off" and _volume_col(df) is None:
        raise ExportIncomplete(
            ...
            "current bos_strategy_export.pine, or take the export with the filter Off — "
    ...
    strat = BosStrategy(cfg).run(_bars_from(df), engine_config=eng)
