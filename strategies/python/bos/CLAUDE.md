# CLAUDE.md — bos (BOS, the break-of-structure continuation bot)

**Purpose:** Standing instructions for this strategy package.
**Sweeps:** `bos_optimization.md`, next to this file — **empty on purpose**, because no sweep
...
when the market closes. ⚠ **Never measured on this bot.** The figures that exist are in
`strategies/notes/flat-before-the-close.md`; all three measured bots lose.
...
taken off `bos_strategy_export.pine` at the shipped defaults, and the Python was configured
...
it is the one whose Pine plots it.** Fixed by making `bos_strategy_export.pine` plot
...
- `docs/BOS_OPTIMIZATION.md` — every run in it, Runs 1–8. **Re-run anything that still
  matters.**
...
- **The EXPORT must carry it too, and it does not come for free.** `bos_strategy_export.pine`
...
into `BosStrategy.exit_stages` now. ⚠ **This is the sharper lesson of the three: a harness
