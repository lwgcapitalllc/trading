...
`strategy()` sources (`strategies/tradingview/CLAUDE.md`) and it does NOT restate what any Python
...
**Last reviewed:** 2026-08-15 — **`m15_playbook.pine` was DELETED** (Aaron's call; see below).
...
🔴 **`m15_playbook.pine` was DELETED on 2026-08-15 and this is the note that says where it
...
`mpc_jarvis.pine` and everything extracted from it (`structure_engine.pine`, every
...
- `indicators/engines/mpc_jarvis.pine` — a full-featured SMC indicator (structure + order blocks + sessions + kill zones + VWAP + liquidity levels + fibonacci + SVP) that Aaron sourced separately. [Detail](../docs/INDICATORS_BUILD_NOTES.md#indicatorsmpcassistantpine)
- `indicators/engines/structure_engine.pine` — a straight extraction of *only* the market-structure logic (external ASH/ASL/BOS/CHoCH/HH/HL/LH/LL + internal iSH/iSL/iBOS/iSOS) from `mpc_jarvis.pine`, with every other feature (OBs, sessions, kill zones, VWAP, liquidity, fibo, SVP) stripped out. [Detail](../docs/INDICATORS_BUILD_NOTES.md#indicatorsstructureenginepine)
...
**Never do**
- Do not use `ta.pivothigh` / `ta.pivotlow` or any fixed-lookback-window pivot method to detect swings in `smc_engine_v2.pine` (the from-scratch rebuild). This does not apply to `mpc_jarvis.pine` / `structure_engine.pine`, which are a separate, intentionally pivot-seeded track — see Key paths above.
