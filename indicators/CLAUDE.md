...
- `indicators/fib_export.pine` — instrumented build for the FIB parity check: the external structure engine (copied verbatim from `structure_engine_export.pine`, internal block omitted) + the Structure fib block lifted from `mpc_assistant.pine` (compute + touched state machine; drawing removed) + `px_fib_*` `plot()` columns. Used to export a CSV that `fibonacci/tools/compare_fib.py` diffs Python-vs-real-Pine. The Structure fib passed at 100% on a `VANTAGE_XAUUSD, 15m` export (9,562 warm bars, 258 touch events). Do not let either part drift from its source (`structure_engine.pine` / `mpc_assistant.pine`).
