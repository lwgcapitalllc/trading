...
**Status:** 🔨 **IN PROGRESS — Stage 2b ~95% validated, Stages 3–4 NOT STARTED** (blocked on chart-by-chart validation, which only a human at TradingView can do). ⚠ **A separate track from the `mpc_jarvis.pine` line the `engines/` Python engines were ported from** — nothing here feeds a live bot.
