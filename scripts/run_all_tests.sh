...
echo "  [1/19] engines / backtest / algos / strategies / smart-money / execution / scripts ..."
...
echo "  [2/19] command-center backend ..."
...
# `npm test` in `command-center/frontend`. ✅ Except the OFFLINE ones, which need nothing running and
# reach nothing live: they are step 19.
#
...
echo "  [6/19] documentation-size guard ..."
...
echo "  [8/19] trade-overlay geometry ..."
if [ -d "command-center/frontend/node_modules" ]; then
...
echo "  [9/19] parameter-condition evaluator ..."
if [ -d "command-center/frontend/node_modules" ]; then
...
echo "  [10/19] period-window rebase ..."
if [ -d "command-center/frontend/node_modules" ]; then
...
echo "  [11/19] instrument search + recents ..."
if [ -d "command-center/frontend/node_modules" ]; then
...
echo "  [12/19] theme colour tokens ..."
if [ -d "command-center/frontend/node_modules" ]; then
...
echo "  [13/19] git-hook bypass guard ..."
...
echo "  [14/19] pine block drift (copies vs the indicator) ..."
...
echo "  [15/19] engine + strategy parity gates (golden exports) ..."
...
echo "  [16/19] generated pine harness in sync with its sources ..."
...
# ── 17. The published clash figures are still measured on today's bots ──────
# 🔴 THE OVERLAP FIGURES IN ROOT CLAUDE.md WENT STALE THREE TIMES, AND EACH TIME A DEFAULT HAD
# MOVED - B-LEG's on 2026-08-06, SOS Fade's dead-market filter on 2026-08-26, its scale-ins and
# re-entry trigger on 2026-09-06 (inside a Command Center commit, where nothing read as an
# entry-logic change). "Re-run it after any entry-logic change" was written down all three times.
# This compares the settings each recorded pair was measured on - strategy AND engine - with the
# settings the bots have today, and goes red whoever moved them. Replays nothing; under a second.
# ⚠ SETTINGS, NOT CODE: a rule changed inside a strategy or an engine still needs a re-run by hand.
...
echo "  [17/19] overlap audit still measured on today's bot settings ..."
...
echo "  [18/19] pine export twins match their parents ..."
...
if [ -d "command-center/frontend/node_modules" ]; then
  if (cd command-center/frontend && npx --no-install playwright test --project=offline --reporter=line); then
    ...
    fail "offline browser specs (command-center/frontend, --project=offline)"
  fi
else
  fail "offline browser specs - command-center/frontend/node_modules missing (run: cd command-center/frontend && npm install)"
fi
...
if [ -n "$FAILED" ]; then
  ...
  echo ""
  echo "  ⚠ Only the OFFLINE browser specs are in this run - the rest need the app up."
