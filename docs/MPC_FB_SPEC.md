# MPC FB — Failed Break (the "Riz" liquidity model) — v0 spec
...
| | A+ SOS Fade (`mpc_sos_fade`) | MPC FB |
|---|---|---|
...
🔴 **It is still an architecture change.** `mpc_sos_fade`'s `Execution` holds ONE position and every
...
| Fib-band entry, leg-origin stop, staged stop, R sizing | ✅ `mpc_sos_fade`'s exit ladder, inheritable |
...
| **Institutional-number grid** | ❌ **new** — small, ~40 lines, plus a Pine overlay. `mpc_assistant.pine` is at the compile-token ceiling, so the paste needs a matching dead-code trim. |
