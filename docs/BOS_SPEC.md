# BOS — Break-of-Structure Continuation Strategy (v1 spec)
...
gap entry to be the losing half of the book. **Read `docs/BOS_OPTIMIZATION.md` → Run 5 before
...
**Target file:** `strategies/tradingview/bos_strategy.pine` (a strategy fork, same pattern as
`b_leg_strategy.pine`).
**Engine source:** `indicators/engines/mpc_jarvis.pine` — the engine block is copied byte-identical
...
Everything the setup needs is already computed in `mpc_jarvis.pine`. Nothing new is invented.
...
`mpc_jarvis.pine` draws and `engines/vwap/` is the canonical Python port of.
...
⚠ **Deliberately NOT added: a slope test.** `d_strategy.pine` carries
...
Reuse the ladder registered in `strategies/python/sos_fade/CLAUDE.md → ## The exit ladder`. Do
...
3. ~~`strategies/tradingview/bos_strategy_export.pine` + a `compare_bos.py` harness~~ — **BOTH DONE.**
   The export twin landed 2026-08-07 (59 plots: the full decision stream, the two anchor
   endpoints, every input as `cfg_*`); `strategies/python/bos/tools/compare_bos.py` landed
   the same day and is unit-tested. ✅ **RUN 2026-08-07 and GREEN** — see below.
4. ~~Only then the Python port under `strategies/python/bos/`~~ — **BUILT 2026-08-07, and
   ...
   `__init__`, its CLAUDE.md, `docs/BOS_OPTIMIZATION.md` and `docs/STRATEGY_WORKFLOW.md`.
...
write-up in `strategies/python/bos/CLAUDE.md`.

⚠ **Green does not backdate.** The port changed to get green, so nothing in
`docs/BOS_OPTIMIZATION.md` is retroactively trustworthy — re-measure what matters.
