...
**Goal:** port the SOS Fade strategy (`strategies/tradingview/sos_fade_strategy.pine`) to a Python bot, backtest it
...
BLOCKED on a TradingView CSV export** of `sos_fade_strategy_export.pine` (5m XAUUSD) from Aaron/brother.
...
- **Strategy logic** → **`strategies/python/sos_fade/`**, fitting the existing "strategies organized
...
**B — The SOS Fade bot** (the strategy logic on top of the canonical engines).
...
- `strategy.py` — `SosFadeStrategy`: the driver (`run(df, warmup=…)` / `step(bar_state)`), collects the
  per-bar decision stream + trade list.
15 offline tests green: `command-center/backend/.venv/bin/python -m pytest strategies/python/sos_fade/tests/`.
...
- **Parity gate (LWG-specific).** `compare_strategy.py`: replay a TradingView export through the
  Python bot and prove the same entries/exits/R as `sos_fade_strategy.pine`. **Nothing is trusted until
...
You and your brother edit `sos_fade_strategy.pine` in TradingView constantly — flipping toggles to see
...
**Three parts:**
1. **`sos_fade_strategy_export.pine`** — an instrumented copy of the strategy (kept byte-identical to the
...
1. Brother edits `sos_fade_strategy.pine` in TradingView.
2. Re-paste it into the repo (git) — same re-paste discipline the engine already uses.
3. Update `sos_fade_strategy_export.pine` to match (only if the trade logic changed).
