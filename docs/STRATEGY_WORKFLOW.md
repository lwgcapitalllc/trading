...
| **Realign** (`realign`) | ✅ | ⚠ stale | ❌ | ❌ | ✅ | ❌ | **No — and this row was MISSING from this table until 2026-09-16, which is how a strategy accumulates six runs of measurements with no gate.** Stages 1 and 5 are done. ⚠ **Stage 2 is marked STALE rather than done**: `realign_strategy.pine` exists but has no input for `strict`, the retest entry, the always-on time stop or flat-by-close — the levers every current number is measured with — so the two trees describe different strategies. Stages 3 and 6 are next; **stage 4 is waiting on a human.** Its lab findings are real and non-trivial (beats matched random entry at z +2.36 on a clean basis) and remain LAB FINDINGS until this row is green. See `strategies/python/realign/realign_optimization.md`. |
