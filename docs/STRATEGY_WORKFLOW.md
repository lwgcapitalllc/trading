...
| **Realign** (`realign`) | ✅ | ⚠ stale | ✅ | ✅ | ✅ | ✅ green (narrow) | **Yes, narrowly** — green 2026-09-16 at the market entry (Run 8) and the retest entry (Run 9), 3.5 months each. **History:** this row was MISSING from this table until 2026-09-16, which is how a strategy accumulates six runs of measurements with no gate. ⚠ **Stage 2 is marked STALE rather than done**: `realign_strategy.pine` has no input for the `strict` pattern (it has the retest entry, the always-on time stop and flat-by-close since 2026-09-16). 🔴 **The first export (2026-09-16) was red, every cause diagnosed** — two port bugs fixed, three Pine-side rules changed — and the second export was green. Record: `realign_optimization.md` Runs 7-8. Its lab findings are real and non-trivial (beats matched random entry at z +2.36 on a clean basis) and remain LAB FINDINGS until this row is green. See `strategies/python/realign/realign_optimization.md`. |
| **H4 sweep** | study only | ✅ | ❌ | ❌ | ❌ | ❌ | No |
| **FFT** (`fft`) | ✅ | — | — | — | ✅ | ✅ **replaced** — matched to the study | **Yes, on the study's terms.** 🔴 **No Pine stage BY DECISION (2026-09-21):** FFT runs on 1-minute bars and the user cannot export 1-minute data from TradingView, so stages 2–4 cannot exist. Stage 6 is `strategies/python/fft/tools/compare_study.py` — the bot against `backtest/tools/fft_first_touch_study.py` on the same PU Prime bars: 157/157 trades and 2,644/2,644 first touches 2020-25, 34/34 and 494/494 the last year. ⚠ The engines are shared, so this proves the RULE LAYER agrees; the engines' own gates against MPC Jarvis and the user's chart check are what cover the rest. `docs/FFT_SPEC.md` → *How it is proven*. |
