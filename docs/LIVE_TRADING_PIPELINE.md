...
### G14 — The overlap audit — **RE-MEASURED 2026-09-10 ON TODAY'S SOS Fade, and it still passes**
...
figure below is the 2026-09-03 re-measurement. **Trade counts and R were never affected** — those
come from the replay, not from the placement.
...
position on **46 bars out of 157,004** — 0.3% of SOS Fade's own hold time, 1.9% of B-LEG's. **ZERO of
those bars were same-side**; all 46 were opposite, i.e. partially hedged. Five SOS Fade trades out of 244
(2.0%) shared a bar with a B-LEG trade, and **none of those five pairs was same-direction**.
...
⚠ **Neither direction is a trend, and the readings are not all comparable.** 2026-08-09 → 2026-09-01:
49 → 45 shared bars, 1 → 0 same-side; the 2026-09-03 reading (46, 0) is the first one measured on a
bot that can fire its re-entries AND a tool that places them correctly, so **it supersedes rather
than continues that series.** Before that the absolute count went UP 27 → 49 while same-side
...
bot sizes off its OWN equity, so on those 46 bars one account would have carried both bots'
per-trade risk at once. The audit says the allocator would rarely have had anything to arbitrate — not
