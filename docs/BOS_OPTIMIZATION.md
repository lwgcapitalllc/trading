...
`strategies/python/sos_fade/sos_fade_optimization.md`.

**Where the old log went.** Runs 1–4 (2026-07-31) lived in `strategies/python/bos/`, which was
...
everything — `sos_fade/CLAUDE.md` records that exact trap, and Run 3 flagged its own
...
3. **Build `bos_strategy_export.pine` + `compare_bos.py`** (spec §10 steps 3–4). Until then
...
⚠ **Still no parity harness.** `bos_strategy_export.pine` exists and a real 20,079-bar export
...
2. **Build `strategies/python/bos/` and `backtest/tools/compare_bos.py`** and get it to exit 0.
