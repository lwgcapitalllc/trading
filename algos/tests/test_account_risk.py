...
def test_a_stop_RATCHETED_INTO_PROFIT_reserves_NOTHING():
    ...
    assert (
        measure_exposure([_pos(direction=-1, entry=3300.0, stop=3280.0)], _gold()).total_ccy == 0.0
    )
    assert (
        measure_exposure([_pos(direction=-1, entry=3300.0, stop=3310.0)], _gold()).total_ccy
        == 250.0
    )
...
def test_two_bots_are_totalled_and_reported_per_magic():
    risk = measure_exposure(
        [_pos(ticket=1, magic=770115), _pos(ticket=2, magic=880220, volume=0.10)], _gold()
    )
...
def _risk(total, per_magic=None, positions=1, resting=0):
    return AccountRisk(
        total_ccy=total, positions=positions, resting=resting, per_magic=per_magic or {}
    )


def test_an_order_that_fits_is_allowed():
    v = check_account_cap(
        new_order_risk_ccy=100.0, open_risk=_risk(50.0), balance=2000.0, cap_pct=10.0
    )
    assert v.allowed
...
def test_the_second_bot_is_REFUSED_when_the_first_holds_the_budget():
    ...
    v = check_account_cap(
        new_order_risk_ccy=200.0,
        open_risk=_risk(200.0, {770115: 200.0}),
        balance=2000.0,
        cap_pct=10.0,
    )
...
def test_an_order_exactly_filling_the_remaining_room_is_ALLOWED():
    ...
    v = check_account_cap(
        new_order_risk_ccy=200.0, open_risk=_risk(0.0, positions=0), balance=2000.0, cap_pct=10.0
    )
    assert v.allowed
...
def test_no_cap_configured_is_a_supported_state_and_says_so():
    ...
    v = check_account_cap(
        new_order_risk_ccy=1e9, open_risk=_risk(0.0), balance=2000.0, cap_pct=None
    )
...
def test_an_unreadable_balance_REFUSES_and_never_reads_as_affordable():
    ...
    v = check_account_cap(new_order_risk_ccy=10.0, open_risk=_risk(0.0), balance=None, cap_pct=10.0)
...
def test_a_zero_or_negative_cap_refuses_LOUDLY_rather_than_silently_blocking_everything():
    ...
    v = check_account_cap(new_order_risk_ccy=1.0, open_risk=_risk(0.0), balance=2000.0, cap_pct=0.0)
