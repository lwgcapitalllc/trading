...
def test_the_2026_08_07_order_is_refused_end_to_end():
    ...
    plan = plan_order(
        ...
        risk_pct=10.0,
        free_margin=2000.0,
        margin_for=lambda lots: lots * 100 * 4286.75 / 500,
    )
    assert not plan.ok
...
def test_a_correctly_configured_symbol_agrees_with_itself_on_every_instrument(spec, entry, stop):
    ...
    plan = plan_order(qty_units=units, entry=entry, stop=stop, spec=spec, point_value=point_value)
    assert plan.ok, getattr(plan, "detail", "")
