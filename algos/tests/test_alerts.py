...
def test_entry_carries_everything_asked_for():
    ...
    msg = alerts.format_entry(strategy="MPC SOS Fade", symbol="XAUUSD.s", direction="LONG",
                              entry=4094.87, stop=4077.62, lots=0.05,
                              risk_usd=200.0, risk_pct=10, when=_WHEN)
...
def test_the_entry_states_the_risk_because_the_exit_will_not():
    ...
    msg = alerts.format_entry(strategy="S", symbol="X", direction="LONG", entry=1.0, stop=0.9,
                              lots=1, risk_usd=200.0, risk_pct=10, when=_WHEN)
...
def test_direction_is_visible_without_opening_the_message():
    ...
    def first(d):
        return alerts.format_entry(strategy="S", symbol="X", direction=d, entry=1.0,
...
def test_the_exit_reason_separates_a_scratch_from_a_real_stop_out():
    ...
    exists to save."""
    scratch = alerts.format_exit(strategy="S", symbol="X", exit_price=4094.87, pnl_usd=-0.62,
                                 r_multiple=-0.01, exit_reason="stop moved to entry", when=_WHEN)
    stopped = alerts.format_exit(strategy="S", symbol="X", exit_price=1.0, pnl_usd=-84.10,
                                 r_multiple=-1.0, exit_reason="stop", when=_WHEN)
...
def test_an_exit_with_no_reason_does_not_print_empty_brackets():
    msg = alerts.format_exit(strategy="S", symbol="X", exit_price=1.0, pnl_usd=1.0,
                             r_multiple=1.0, when=_WHEN)
...
def test_no_message_carries_a_timestamp():
    ...
    entry = alerts.format_entry(strategy="S", symbol="X", direction="LONG", entry=1.0, stop=0.9,
                                lots=1, when=_WHEN)
    exit_ = alerts.format_exit(strategy="S", symbol="X", exit_price=1.0, pnl_usd=1.0,
                               r_multiple=1.0, when=_WHEN)
...
def test_every_message_opens_with_an_icon_a_label_and_stays_short():
    ...
    msgs = [
        alerts.format_entry(strategy="MPC SOS Fade", symbol="XAUUSD.s", direction="LONG",
                            entry=4094.87, stop=4077.62, lots=0.05, risk_usd=200.0,
                            risk_pct=10, when=_WHEN),
        alerts.format_exit(strategy="S", symbol="X", exit_price=1.0, pnl_usd=1.0,
                           r_multiple=1.0, when=_WHEN),
    ]
    for msg in msgs:
