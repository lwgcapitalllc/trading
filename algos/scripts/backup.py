...
Restore (new VPS): git clone -b backups <repo> C:\trading-backup
then manually copy files to their C:\trading\algos paths.

ONE-TIME VPS SETUP (run once after first deploy):
  python C:/trading/algos/scripts/backup.py --setup
...
Run: python C:/trading/algos/scripts/backup.py
"""
...
def setup():
    ...
    git worktree pointing to it at C:\\trading-backup.

    Run: python C:/trading/algos/scripts/backup.py --setup
    """
    ...
    print("Setup complete. Run 'python C:/trading/algos/scripts/backup.py' to take the first backup.")
...
def backup():
    ...
    if not BACKUP_WORKTREE.exists():
        ...
        print("Run: python C:/trading/algos/scripts/backup.py --setup")
        return
