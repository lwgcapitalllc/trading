...
- **Deploy:** `git push` on Mac → `ssh forexvps "cd C:\trading && git pull origin main"` → `algo restart`
...
- **Backup:** `scripts/backup.py` runs twice daily (midnight + noon CT) via SYS_BACKUP. Commits VPS runtime
  data to the `backups` orphan branch via a git worktree at `C:\trading-backup`. Never touches `main`,
