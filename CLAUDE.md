...
**Status:** Active — four apps, nine canonical engines, and tooling in various stages of production.
...
### engines/fibonacci/
Canonical fib engine. Turns `engines/market_structure/` output (public `StructureSnapshot` only) into fib LEVEL EVENTS — the first-touch of each level (E1–E4 entries, TP1–TP5 targets, 1.0) — via three fib state machines (Structure "FFT", Sniper, Macro) ported from `indicators/mpc_assistant.pine`. All three are unit-tested and validated at 100% Pine parity on real XAUUSD exports. Full rules in `engines/fibonacci/CLAUDE.md`.

### engines/order_blocks/
Canonical order-block engine. Turns `engines/market_structure/` output into order-block EVENTS — a supply/demand zone per structure break, its mitigation (tap), and FIFO eviction past 6 per direction; a SIBLING of `engines/fibonacci/`, consuming the structure engine's public output only. Ported from `mpc_assistant.pine`, unit-tested (12 hand-traced tests), 100% Pine parity on real `VANTAGE_XAUUSD` exports. Full rules in `engines/order_blocks/CLAUDE.md`.
...
### engines/liquidity/
Canonical liquidity-levels engine. Turns the bar stream into liquidity LEVEL EVENTS — prev day/week/month high & low, prev week close, the H4 sweep high/low, and Asia/London/NY session high & low, each with sweep/break mitigation; composes `engines/sessions/` for the session levels. **Non-repainting by Aaron's explicit decision (2026-07-05): every HTF level uses the PREVIOUS completed period only — never the current period's forecast.** Unit-tested (15 hand-traced tests), 100% Pine parity on a real `VANTAGE_XAUUSD, 5m` export; XAUUSD trading day opens 18:00 NY (baked-in default). Full rules in `engines/liquidity/CLAUDE.md`.
...
Canonical Session Volume Profile engine — the Asia point-of-control ("MV" line): on each Asia session close it builds a 100-row volume profile over the session range, reports the highest-volume row's mid-price as the POC, and marks it confirmed when price first straddles it. Composes `engines/sessions/`; needs the **volume** feed. Unit-tested (12 hand-traced tests), 100% Pine parity on a real `VANTAGE_XAUUSD, 5m` export. The **last SMC-port engine — the extraction roadmap is complete.** Full rules in `engines/session_volume_profile/CLAUDE.md`.
