...
🔴 **WHAT ACTUALLY BLOCKS A SECOND BOT IS A NUMBER, NOT A FEATURE: `exec_risk_pct` is 10.0 and the account cap is 10.0**, so one full-size SOS Fade trade fills the entire budget and a second bot is refused until that room comes back. That is a decision about how to split the pool, not code to write.
