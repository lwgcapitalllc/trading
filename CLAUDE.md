...
**The extreme-leg bot does not clash with A+ either — MEASURED 2026-09-01, and this is the first audit across two different bar frames.** Over 470,995 M5 bars of PU Prime `XAUUSD.p` (2020-01-01 → 2026-08-23) A+ (15m) and the extreme-leg bot (5m) held a position at the same time on **1,066 bars** — 3.6% of A+'s hold time — of which **ZERO were same-side** (all 1,066 opposite, i.e. partly hedged). A+ 156 trades / +131.77R, extreme leg 132 / +57.10R; 7 trade pairs touch at all, **none** same-direction, and **not one same-direction entry lands within four hours of the other's** in 6.6 years. Monthly R correlation **+0.025** over 79 months — same floor caveat as above. ⚠ **This is a WEAKER claim than the A+/B-LEG one, not an equal one: the extreme-leg bot has no parity gate result behind these trades yet**, so the figures describe the Python port rather than a proven twin of the Pine. ⚠ **It does not retire the allocator** — peak concurrent positions is still 2, so one account carries 2× `exec_risk_pct` on those 1,066 bars, and that is 24× more shared bars than B-LEG contributes. ⚠ **A bar count cannot be compared across the two audits** — one is counted in 15-minute bars and this one in 5-minute bars; compare the *percentages* of hold time. Why the shared time axis is the finer frame's own index rather than a clock: `backtest/CLAUDE.md` → `tools/overlap_audit.py`.
