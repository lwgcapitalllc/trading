...
|---|---|
| `strategies/` | Strategy source by runner platform — NT8 `.cs`, MT5 `.mq5`, Python packages, and since **2026-09-02** the Pine `strategy()` files in `tradingview/` (moved in from `indicators/strategies/`; the two scratch ideas dropped to `tradingview/research/`). ⚠ **Nothing under `tradingview/` is scanned or deployed** — the lab's scanner globs `.cs` and `.mq5` only. Python ports with a GREEN parity gate: `sos_fade`, `b_leg`, and `bos` (**narrow** — the gap ladder never ran). **`realign`'s gate ran once (2026-09-16) and is RED** pending a second export — so every number it has produced is still a lab finding. The end-to-end process, and which step only a human can do, is `docs/STRATEGY_WORKFLOW.md`. |
