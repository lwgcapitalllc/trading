...
- Build a second structure engine, fib engine, order-block engine, or sessions engine anywhere — `engines/market_structure/engine.py`, `engines/fibonacci/`, `engines/order_blocks/` and `engines/sessions/` are the canonical implementations; all consumers import from them
