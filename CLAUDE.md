...
`check_guard.py`. ⚠ **What it does NOT do is arrive in time to prevent the crossing**, and the
before-the-edit half still cannot, because it would have to reason about the size the file is
ABOUT to be rather than the size it is.
...
than a measurement. ⚠ **Chasing the distribution upward is how a limit stops meaning anything** —
the honest reading is that the repo grew into the number, not that the number is wrong.
