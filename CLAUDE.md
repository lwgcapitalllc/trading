...
### fibonacci/
Canonical fib engine. Turns `market_structure/` output into fib LEVEL EVENTS — the first-touch of each fib level (E1–E4 entries, TP1–TP5 targets, 1.0) — for entries, take-profits, and setup grading. One shared geometry core serves all three fibs (Structure, Sniper, Macro); each is a small state machine ported line-by-line from `indicators/mpc_assistant.pine`. Consumes the structure engine's public output only (never its internals) via a `StructureSnapshot`. The Structure fib ("FFT") is ported and validated at 100% Pine parity on a real XAUUSD 15m export; the Sniper fib is ported and unit-tested with its parity harness wired but not yet run against a fresh export; the Macro fib is not yet ported. Emits events, not visuals. This is the single implementation — do not build another. Full rules in `fibonacci/CLAUDE.md`.
