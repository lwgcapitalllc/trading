...
Canonical order-block engine. Turns `engines/market_structure/` output into order-block EVENTS — a supply/demand zone per structure break, its mitigation (tap), and FIFO eviction past 6 per direction; a SIBLING of `engines/fibonacci/`, consuming the structure engine's public output only. Ported from `mpc_assistant.pine`, unit-tested (12 hand-traced tests), 100% Pine parity re-confirmed after the 2026-07-12 structure re-sync (STALE-BY-INPUT, same as the fibs; fresh 5m export, exit 0). Full rules in `engines/order_blocks/CLAUDE.md`.
