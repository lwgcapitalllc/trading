...
`algos/`, `smart-money/`, and `command-center/` are fully independent from each other. `engines/regime/` and `engines/market_structure/` are shared libraries imported by `algos/` (each via a thin shim in `algos/shared/`); `engines/regime/` is also imported by `command-center/` (directly). `engines/fibonacci/` is a shared library downstream of `engines/market_structure/` — it consumes that engine's public output (a `StructureSnapshot`) and will get an `algos/shared/` shim when a bot first uses it. `engines/order_blocks/` is a sibling of `engines/fibonacci/` — it also consumes `engines/market_structure/`'s public output directly (via its own decoupled `StructureSnapshot`, never fibonacci) and will likewise get an `algos/shared/` shim when a bot first uses it. `engines/sessions/` is a standalone, time-driven shared library (input = the bar's UTC timestamp, not `engines/market_structure/` output); it is the prerequisite for the session-scoped parts of the Liquidity engine (now built), and will get an `algos/shared/` shim when a bot first uses it. `engines/liquidity/` is another time-driven library that CONSUMES `engines/sessions/` (composes it) for its session high/low levels and reconstructs the day/week/month/H4 levels from the bar stream; it depends on nothing else and will get an `algos/shared/` shim when a bot first uses it. `engines/vwap/` is a standalone, time-driven library (input = the bar's UTC timestamp + high/low/close + **volume** — the first engine to need volume) that reconstructs its trading-day anchor directly (the same 18:00-NY boundary the liquidity daily level uses) rather than composing `engines/sessions/`; it depends on nothing else and will get an `algos/shared/` shim when a bot first uses it. `engines/svp/` is a time-driven library that CONSUMES `engines/sessions/` (composes it) for the Asia session window/edges and — like `engines/vwap/` — needs the bar's **volume**; on each Asia close it builds a volume profile whose highest-volume row is the session point-of-control (the "MV" line); it depends on nothing else and will get an `algos/shared/` shim when a bot first uses it. `engines/news/` is a standalone, time-driven library (input = the bar's UTC timestamp) that is **off-roadmap — NOT a `mpc_assistant.pine` port**: it turns an external economic-calendar feed (the free Forex Factory feed) into trade-BLACKOUT events so a bot can veto trading around scheduled macro news; it depends on nothing and will get an `algos/shared/` shim when a bot first uses it. `strategies/` is consumed by `command-center/` (scanner + deploy) and deployed to the VPS (NT8 strategy folder).
...
### engines/vwap/
Canonical session-VWAP engine. Turns the bar stream into the VWAP value — a volume-weighted running mean of `hlc3`, re-anchored each trading day — plus a **derived** close-vs-line cross. Ported line-by-line from `indicators/mpc_assistant.pine`'s one VWAP line (`vwapValue = ta.vwap(hlc3)`). The **first engine to need a volume column** in the feed (XAUUSD tick volume — exactly what the Pine `ta.vwap` reads). A time-driven engine that reconstructs the trading-day anchor DIRECTLY (the **same** 18:00-NY boundary the liquidity daily level uses, same forward-shift calibration knob), so it does not compose the sessions engine. Ported, unit-tested (13 hand-traced tests), and validated at **100% Pine parity on a real `VANTAGE_XAUUSD, 5m` export** (6,973 bars; both fields — VWAP value + trading-day anchor pulse — `--htf-rollover 18 --warmup 90`, exit 0; harness: `indicators/vwap_export.pine` + `engines/vwap/tools/compare_vwap.py`). The parity check uses a **relative** tolerance (1e-6) because the value is a cumulative sum that drifts at float-rounding level — unlike the copied-value level engines. The `value`/anchor are Pine-validated; the cross is a derived convenience (the source only draws the line). Emits events, not visuals. This is the single implementation — do not build another. Full rules in `engines/vwap/CLAUDE.md`.

### engines/svp/
Canonical Session Volume Profile engine — the Asia point-of-control ("MV" line). A time-driven library that CONSUMES `engines/sessions/` (composes it) for the Asia session window/edges + running high/low, and — like `engines/vwap/` — needs the bar's **volume**. On each Asia session (2000-0500 GMT-4) close it builds a 100-row volume profile over the session range, spreads each bar's volume across the rows its high/low span, and reports the mid-price of the highest-volume row as the POC; the "MV slot" then marks that POC confirmed the first time price straddles it. Ported line-by-line from `mpc_assistant.pine`'s SVP block (line ~2554) + MV slot (line ~2772). Two Pine quirks ported exactly: the session-close bar is folded into the profile, and the bull/bear two-array newest-first summation is kept (float addition is not associative — collapsing it could flip a near-tie POC row). Ported, unit-tested (12 hand-traced tests), and validated at **100% Pine parity on a real `VANTAGE_XAUUSD, 5m` export** (7,608 bars; all 3 fields — POC price, form pulse, sweep state — match, `--warmup 116`, exit 0; harness: `indicators/svp_export.pine` + `engines/svp/tools/compare_svp.py`). The POC uses an **exact** (1e-6) tolerance — deterministic on the copied session H/L + integer volume, so bit-identical (unlike VWAP's cumulative value). The **last SMC-port engine — the extraction roadmap is now complete.** Emits events, not visuals. This is the single implementation — do not build another. Full rules in `engines/svp/CLAUDE.md`.
...
- Build a second structure engine, fib engine, order-block engine, sessions engine, liquidity engine, VWAP engine, SVP engine, or news/economic-calendar engine anywhere — `engines/market_structure/engine.py`, `engines/fibonacci/`, `engines/order_blocks/`, `engines/sessions/`, `engines/liquidity/`, `engines/vwap/`, `engines/svp/` and `engines/news/` are the canonical implementations; all consumers import from them
