...
def ev(y, mo, d, h, mi, currency="USD", impact=Impact.HIGH, title="CPI"):
    return NewsEvent(timestamp_ms=ms(y, mo, d, h, mi), currency=currency, impact=impact, title=title)
...
def test_no_coverage_forces_filter_off():
    e = ev(2026, 7, 6, 12, 30)
    eng = NewsEngine([e], policy=NewsPolicy.usd(), covered_ranges=[(ms(2026, 7, 6, 0, 0), ms(2026, 7, 6, 23, 59))])
...
def test_active_prefers_higher_impact_on_overlap():
    ...
    eng = NewsEngine([med, high], policy=NewsPolicy(currencies=frozenset({"USD"}), min_impact=Impact.MEDIUM))
...
def test_holiday_is_reported_but_not_blocked_by_default():
    ...
    eng = NewsEngine([_holiday(2026, 7, 3)], policy=NewsPolicy.usd())  # block_holidays defaults False
...
def test_holiday_reporting_respects_currency_filter():
    ...
    eng = NewsEngine([_holiday(2026, 7, 3, currency="GBP")],
