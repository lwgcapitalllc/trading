...
def test_internal_swing_adopted_as_pull_anchor():
    ...
    snap = StructureSnapshot(
        ...
        asl_loc=0,
        direction=1,
        i_confirmed_low_price=95.0,
        i_confirmed_low_loc=5,  # deeper than asl=100
    )
...
def test_internal_swing_not_adopted_when_less_extreme():
    ...
    snap = StructureSnapshot(
        ...
        asl_loc=0,
        direction=1,
        i_confirmed_low_price=103.0,
        i_confirmed_low_loc=5,  # shallower than asl=100
    )
    ev = fib.update(high=109.0, low=108.0, snap=snap)
...
def test_extend_changed_bar_skips_touch_checks():
    ...
    assert (
        abs(ev.levels["E1"] - (112.0 - 12.0 * 0.618)) < 1e-9
    )  # levels recomputed off the moved anchor
...
def test_half_reached_inbound_05_is_ungated():
    ...
    assert "TP1" not in {
        t.level for t in ev.touched
    }  # the TP1 target did not fire (needs the gate)
