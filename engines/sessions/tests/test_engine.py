...
from sessions.engine import _in_window, _pine_dayofweek, _resolve_tz
...
def test_killzones_ny_time():
    ...
    assert (
        SessionEngine().update(0, ms("America/New_York", 2024, 7, 2, 11, 0), 1, 0).in_kz1 is False
    )
    assert (
        SessionEngine().update(0, ms("America/New_York", 2024, 7, 2, 11, 45), 1, 0).in_kz2 is True
    )
    assert (
        SessionEngine().update(0, ms("America/New_York", 2024, 7, 2, 12, 14), 1, 0).in_kz2 is True
    )
    assert (
        SessionEngine().update(0, ms("America/New_York", 2024, 7, 2, 12, 15), 1, 0).in_kz2 is False
    )
    assert (
        SessionEngine().update(0, ms("America/New_York", 2024, 7, 2, 13, 30), 1, 0).in_kz3 is True
    )
    assert (
        SessionEngine().update(0, ms("America/New_York", 2024, 7, 2, 13, 31), 1, 0).in_kz3 is False
    )
