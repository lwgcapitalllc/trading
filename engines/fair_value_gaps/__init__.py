...
Turns the bar stream into fair-value-gap LEVEL EVENTS — a price void left by a clean 3-candle
displacement, and the bar it is later mitigated (tapped) on. Colours, boxes and the directional
visibility filter are out of scope; this is the trading-signal layer, not the drawing layer.
...
Public API:
    ...
    fvg = FairValueGapEngine()          # max_count=3, min_ticks=0 — the Pine defaults
    ...
    for g in ev.mitigated:              # gaps tapped out THIS bar
        ...
