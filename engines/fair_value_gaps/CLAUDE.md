...
fvg = FairValueGapEngine()   # engine.py's DEFAULT_* constants — the indicator's BELOW-15m settings
...
## Speed — the cap's exemption scan is skipped when nothing can be over the cap (2026-09-16)

When the whole live list fits under the cap, no gap can be evicted, so the per-bar exemption count
is skipped. MEASURED by CPU time over 31,095 bars with an EQ level passed: 1.4 s → 0.9 s, event
output identical, golden gate green.
