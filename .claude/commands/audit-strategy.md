...
5. **Fixing a divergence (only if asked to fix, not just report):** change the Python to match the Pine, re-run until exit 0. Never change the Pine to match the Python — the Pine is the source of truth. Add a unit test in `strategies/python/mpc_sos_fade/tests/` capturing the rule you just fixed, so it can't regress. Then remind Aaron that a real fix isn't done until `compare_strategy.py` is exit 0 on the fresh export.
